...
// --- INSTALL APP COMPONENT (ĐÃ SỬA ĐỂ LUÔN HIỆN) ---
const InstallAppItem: React.FC<{
    deferredPrompt: any;
    onInstall: () => void;
}> = ({ deferredPrompt, onInstall }) => {
    // ĐÃ COMMENT DÒNG DƯỚI ĐỂ NÚT LUÔN HIỆN CHO BẠN TEST
...
export default function GameProfile() {
  ...
  // --- EFFECT MỚI: LẤY SỰ KIỆN TỪ WINDOW (DO INDEX.HTML BẮT) ---
  useEffect(() => {
    // 1. Kiểm tra xem index.html đã bắt được sự kiện chưa (tránh Race Condition)
    if ((window as any).deferredPrompt) {
      console.log("Found deferredPrompt in window global scope");
      setInstallPrompt((window as any).deferredPrompt);
    }

    // 2. Phòng hờ: Nếu sự kiện bắn chậm (sau khi vào Profile mới bắn)
    const handler = (e: any) => {
      e.preventDefault();
      (window as any).deferredPrompt = e; // Lưu lại global luôn
      setInstallPrompt(e);
      console.log("Captured install prompt inside Profile component");
    };
  ...
  // --- HÀM XỬ LÝ CÀI ĐẶT MỚI ---
  const handleInstallApp = async () => {
    // Ưu tiên lấy từ state, nếu không có thì lấy từ window global
    const promptEvent = installPrompt || (window as any).deferredPrompt;

    if (!promptEvent) {
        alert("Không tìm thấy sự kiện cài đặt. Hãy thử tải lại trang hoặc kiểm tra xem App đã được cài chưa.");
        return;
    }
