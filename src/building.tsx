// --- START OF FILE equipment.tsx (4).txt ---
...
// ==================================================================
// ============ START OF UPDATED ItemCard COMPONENT ==========
// ==================================================================
const ItemCard = memo(({ ownedItem, onClick, onUpgrade, isEquipped, isProcessing, gold }: { 
    ownedItem: OwnedItem, 
    onClick: (item: OwnedItem) => void, 
    onUpgrade: (item: OwnedItem) => void, 
    isEquipped: boolean, 
    ...
    const upgradeCost = getUpgradeCost(itemDef, ownedItem.level);
    const canAfford = gold >= upgradeCost;
    const isUpgradable = !!itemDef.stats;

    const baseClasses = "relative w-full p-3 rounded-lg border-2 flex items-center gap-3 transition-all duration-200";
        ...
        <div className={`${baseClasses} border-slate-700 bg-slate-900/70 ${interactivity}`} onClick={!isEquipped && !isProcessing ? () => onClick(ownedItem) : undefined}>
            ...
            <div className="flex-grow min-w-0">
                <div className="flex justify-between items-start">
                    <h3 className={`text-base font-bold truncate ${getRarityTextColor(itemDef.rarity)}`}>{itemDef.name}</h3>
                    <span className={`flex-shrink-0 ml-2 px-2 py-0.5 text-xs font-bold rounded-full bg-slate-800 border ${getRarityColor(itemDef.rarity)} ${getRarityTextColor(itemDef.rarity)}`}>{itemDef.rarity}</span>
                </div>
                <p className="text-xs text-slate-400 capitalize">{itemDef.type}</p>
            </div>
            ...
            {isUpgradable && (
                 <div className="flex-shrink-0 flex flex-col items-end gap-1.5">
                    ...
                    <button 
                        ...
                        disabled={!canAfford || isProcessing || isEquipped} 
                        ...
                        ${!canAfford || isProcessing || isEquipped
...
// ================================================================
// ============ END OF UPDATED ItemCard COMPONENT ==========
// ================================================================
...
export default function EquipmentScreen({ onClose, gold, equipmentPieces, ownedItems, equippedItems, onInventoryUpdate }: EquipmentScreenProps) {
    ...
    const handleUpgradeItem = useCallback(async (itemToUpgrade: OwnedItem) => {
        ...
        try {
            ...
            // If the upgraded item is the one currently selected in the detail modal, update the modal's state
            if (selectedItem && selectedItem.id === itemToUpgrade.id) {
                setSelectedItem(updatedItem);
            }
        } catch(error: any) { showMessage(`Lỗi: ${error.message || 'Nâng cấp thất bại'}`); } finally { setIsProcessing(false); }
    }, [isProcessing, gold, ownedItems, equippedItems, onInventoryUpdate, showMessage, selectedItem]);
