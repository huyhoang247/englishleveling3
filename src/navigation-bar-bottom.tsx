...
// Hàm xử lý ẩn/hiện thanh điều hướng const toggleVisibility = () => { setIsVisible((prev) => !prev); };

return ( <React.Fragment> {/* Render content theo tab */} <div className="flex-1"> {activeTab === "profile" && <Profile />} {activeTab === "story" && <VerticalFlashcardGallery />} </div>
...
<div className="fixed bottom-0 left-0 right-0 flex flex-col items-center">
    ...
    <div
      className={`bg-black bg-opacity-85 backdrop-blur-md shadow-2xl rounded-t-2xl border-t border-gray-800 w-full transition-all duration-300 ease-in-out overflow-hidden ${isVisible ? "max-h-24 opacity-100" : "max-h-0 opacity-0"}`}
    >
        ...
        {tabs.map((tab, index) => {
          ...
          return (
            <div key={tab.id} className="flex-1 relative flex justify-center items-center">
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${tab.gradient} rounded-xl blur-sm transition-opacity duration-300 ease-in-out ${isActive ? "opacity-15" : "opacity-0"}`}
                />
                <div
                  className={`p-2 rounded-full transition-all duration-300 ease-in-out transform ${isActive ? `bg-gradient-to-br ${tab.gradient} shadow-lg` : "bg-transparent"}`}
                >
                  <Icon size={20} color={isActive ? "#ffffff" : "#9ca3af"} strokeWidth={isActive ? 2.5 : 2} />
                </div>
              </button>
              {index < tabs.length - 1 && <div className="absolute right-0 top-1/2 -translate-y-1/2 h-8 w-px bg-gray-800" />}                 
            </div>
          );
        })}
      </div>
