...
// --- ICONS (có thể import từ context hoặc file chung) ---
...
const formatNumber = (num: number) => { if (num < 1000) return num.toString(); if (num < 1000000) return `${(num / 1000).toFixed(1).replace(/\.0$/, '')}K`; if (num < 1000000000) return `${(num / 1000000).toFixed(1).replace(/\.0$/, '')}M`; return `${(num / 1000000000).toFixed(1).replace(/\.0$/, '')}B`; };

// --- COMPONENT STAT CARD (không thay đổi) ---
import { calculateUpgradeCost, getBonusForLevel } from './upgrade-stats.context.tsx'; // Import từ context
...
// --- COMPONENT HIỂN THỊ CHÍNH (VIEW) ---
const UpgradeStatsView = ({ onClose }: { onClose: () => void }) => {
  const { 
    isLoading, stats, animatedGold, message, upgradingId, toastData,
    totalHp, totalAtk, totalDef, prestigeLevel, totalLevels, currentProgress, 
    progressPercent, handleUpgrade 
  } = useUpgradeStats();
          ...
          <div className="w-full px-2 mb-8">
            ...
            <div className="relative w-full h-7 bg-black/40 rounded-full border-2 border-slate-700/80 p-1 shadow-inner backdrop-blur-sm">
                ...
                <div className="absolute inset-0 flex justify-end items-center px-4 text-sm text-white text-shadow-sm font-bold">
                    <span>{currentProgress}<span className="text-slate-300">/ 50</span></span>
                </div>
            </div>
          </div>
