...
// Gem Icon Component using Image
  ...
  <div className={`flex items-center justify-center ${className}`} style={{ width: size, height: size }} {...props}>
    <img
      ...
      onError={(e) => {
        const target = e.target as HTMLImageElement;
        target.onerror = null; // Prevent infinite loop
        target.src = "https://placehold.co/24x24/FFD700/000000?text=Key"; // Placeholder image (Gold color)
      }}
    />
  </div>
);
...
class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    console.error("Uncaught error in component:", error, errorInfo); // Changed log message
  }
    ...
    if (this.state.hasError) {
      ...
      return this.props.fallback || (
        <div className="text-red-500 p-4 bg-red-100 border border-red-400 rounded">
          <p>Có lỗi xảy ra khi hiển thị nội dung.</p> {/* Updated message */}
...
interface GameObstacle {
  ...
  canDropKey?: boolean; // NEW: Flag to indicate if this obstacle type can drop a key
}
...
// NEW: Define interface for Key
interface GameKey {
    id: number;
    x: number; // Horizontal position in %
    y: number; // Vertical position in %
    speed: number; // Speed for horizontal movement (left)
}

// Define interface for Cloud with image source
interface GameCloud {
  id: number;
  x: number; // Horizontal position in %
  y: number; // Vertical position in %
...
export default function ObstacleRunnerGame({ className }: ObstacleRunnerGameProps) {
  ...
  const [activeKeys, setActiveKeys] = useState<GameKey[]>([]); // Array of active keys on the ground
  const [defeatedEnemiesCount, setDefeatedEnemiesCount] = useState(0); // Count of defeated enemies since last key drop
  ...
  // Ref for the main game loop interval
  const gameLoopIntervalRef = useRef<NodeJS.Timeout | null>(null); // Specify type

  // Ref to store the timestamp when shield cooldown started
  ...
  // Obstacle types with properties (added base health and canDropKey)
  const obstacleTypes: Omit<GameObstacle, 'id' | 'position' | 'health' | 'maxHealth'>[] = [
    // Lottie Obstacle Type 1
    {
      ...
      canDropKey: true, // This obstacle type can drop a key
    },
    // Lottie Obstacle Type 2
    {
  ...
  const startGame = () => {
    ...
    setActiveCoins([]); // Reset active coins
    // NEW: Reset key states
    setKeysCollected(0);
    setActiveKeys([]);
    setDefeatedEnemiesCount(0);
    ...
    setIsStatsFullscreen(false); // Ensure full-screen stats is closed

    setCoins(357); // Reset coin count to initial value
    setDisplayedCoins(357); // Reset displayed coin count
    setGems(42); // Reset gems count to initial value
    ...
    if (obstacleTypes.length > 0) {
        ...
        initialObstacles.push({
          ...
          maxHealth: firstObstacleType.baseHealth, // Set max health
           canDropKey: firstObstacleType.canDropKey, // Include canDropKey property
        });
        ...
        for (let i = 1; i < 5; i++) {
          ...
          initialObstacles.push({
            ...
            canDropKey: obstacleType.canDropKey, // Include canDropKey property
          });
        }
    }
    ...
    // Schedule the next obstacle after the initial ones
    // This timer should only run when the game is active and not paused
  ...
  const generateParticles = () => {
    ...
    for (let i = 0; i < 2; i++) { // Generate 2 particles at a time
      newParticles.push({
        id: Date.now() + i, // Unique ID
        x: 5 + Math.random() * 5, // Starting X position (near character, using 5%)
      ...
      if (obstacleTypes.length > 0) {
          for (let i = 0; i < obstacleCount; i++) {
            ...
            newObstacles.push({
              ...
              canDropKey: randomObstacleType.canDropKey, // Include canDropKey property
            });
          }
      }
  ...
  // Move obstacles, clouds, particles, Coins, and NEW: Keys, and detect collisions
  ...
  useEffect(() => {
    ...
    if (!gameLoopIntervalRef.current) {
        gameLoopIntervalRef.current = setInterval(() => {
            ...
            setObstacles(prevObstacles => {
                ...
                // Adjusted characterXPercent to move character backward
                ...
                return prevObstacles
                    .map(obstacle => {
                        ...
                        // Check if obstacle health is 0 or less (defeated) AND it hasn't collided (to avoid double counting)
                        // Also check if it's a type that can drop a key
                        if (obstacle.health <= 0 && !collisionDetected && obstacle.canDropKey) {
                           // Increment defeated enemy count
                           setDefeatedEnemiesCount(prev => {
                                const newCount = prev + 1;
                                // Check if it's time to drop a key
                                if (newCount >= 10) {
                                    // Random chance to drop a key (1 in 10 to 1 in 20)
                                    const dropChance = 1 / (Math.random() * 11 + 10); // Random number between 10 and 20
                                    if (Math.random() <= dropChance) {
                                        console.log("Key dropped!");
                                        // Add a new key to the activeKeys state
                                        setActiveKeys(prevKeys => [
                                            ...prevKeys,
                                            {
                                                id: Date.now() + Math.random(), // Unique ID
                                                x: obstacle.position, // Drop key at obstacle's current position
                                                y: GROUND_LEVEL_PERCENT, // Drop key on the ground
                                                speed: speed // Key moves at the same speed as obstacles
                                            }
                                        ]);
                                        return 0; // Reset defeated enemy count after dropping a key
                                    }
                                }
                                return newCount; // Return the new count if no key was dropped
                           });
                        }
                    ...
                    .filter(obstacle => {
                        ...
                        // Now also filter out obstacles that have the 'collided' flag set OR health is 0 or less
            ...
            // --- NEW: Move keys and detect collisions ---
            setActiveKeys(prevKeys => {
                const gameContainer = gameRef.current;
                if (!gameContainer) return prevKeys; // Return early if ref is not available
                ...
                // Approximate key size in pixels (assuming KeyIcon is roughly 24x24px)
                const keySize_px = 24;


                return prevKeys
                    .map(key => {
                        let newX = key.x - key.speed; // Move key left

                        let collisionDetected = false;
                         // Key position in pixels relative to the bottom-left of the game container
                        const keyX_px = (newX / 100) * gameWidth; // Use newX for collision check
                         // Key Y is relative to GROUND_LEVEL_PERCENT from bottom
                        const keyBottomFromTop_px = gameHeight - (key.y / 100) * gameHeight; // Key bottom edge from top of container
                        const keyTopFromTop_px = keyBottomFromTop_px - keySize_px; // Key top edge from top of container


                        // Check for collision with character's bounding box
                         if (
                            characterRight_px > keyX_px &&
                            characterLeft_px < keyX_px + keySize_px &&
                            characterBottomFromTop_px > keyTopFromTop_px && // Character bottom is below key top
                            characterTopFromTop_px < keyBottomFromTop_px // Character top is above key bottom
                        ) {
                            collisionDetected = true;
                            setKeysCollected(prev => prev + 1); // Increment key count
                    ...
                    .filter(key => {
                        // Remove key if it was collected or moved far off-screen (left)
                        const isOffScreen = key.x < -20;
                        return !key.collided && !isOffScreen; // Keep key if not collected and not off-screen
                    });
            });
    ...
    // Add isStatsFullscreen and keysCollected to dependency array
  }, [gameStarted, gameOver, jumping, characterPos, obstacleTypes, isStatsFullscreen, coins, isShieldActive, keysCollected]); // Added isStatsFullscreen and keysCollected to dependency array
  ...
  useEffect(() => {
    return () => {
      ...
      // Clear the main game loop interval on unmount
      if (gameLoopIntervalRef.current) {
          clearInterval(gameLoopIntervalRef.current);
      }
    };
    ...
    return (
      <div
        ...
        style={{
          ...
          // Adjusted left position to move character backward
  ...
  const renderObstacle = (obstacle: GameObstacle) => {
    ...
    switch(obstacle.type) {
      ...
      case 'lottie-obstacle-1':
        ...
        break;
      // Case for Lottie Obstacle Type 2
    ...
    return (
      <div
        ...
        style={{
          ...
          bottom: `calc(${GROUND_LEVEL_PERCENT}% - ${obstacleHeightPx}px)`, // Position obstacle base at ground level
          left: `${obstacle.position}%`, // Horizontal position
          // Add a slight transformY to position the obstacle correctly on the ground
          transform: 'translateY(1px)' // Adjust as needed for visual alignment
        }}
      >
         {/* --- NEW: Key Icon Above Obstacle --- */}
        {/* Only show the key icon if the obstacle can drop a key and is alive */}
        {obstacle.canDropKey && obstacle.health > 0 && (
             <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-6 h-6"> {/* Position above obstacle */}
        ...
        {/* Position the health bar above the obstacle element */}
        {obstacle.health > 0 && ( // Only show health bar if obstacle is alive
    ...
    return particles.map(particle => (
      <div
        ...
        style={{
          ...
          left: `calc(5% + ${particle.x}px)`, // Horizontal position relative to character (using 5%)
      ...
      <div
        ...
        className="absolute w-20 h-20 flex flex-col items-center justify-center pointer-events-none z-20" // Adjusted size, z-index
         style={{
          ...
          // Adjusted left position to move shield forward
  ...
  const renderCoins = () => {
    ...
    const renderKeys = () => {
        ...
        const keySize_px = 24;
        const gameContainer = gameRef.current;
        const gameHeight = gameContainer ? gameContainer.offsetHeight : 0;
        const groundLevelPx = (GROUND_LEVEL_PERCENT / 100) * gameHeight;


        return activeKeys.map(key => (
          <div
            key={key.id} // Unique key
            className="absolute w-6 h-6" // Container size for KeyIcon
            style={{
              bottom: `calc(${GROUND_LEVEL_PERCENT}% + 0px)`, // Position on the ground
              left: `${key.x}%`, // Position based on current X (%)
              transform: 'translate(-50%, 0)', // Center horizontally above the ground
              pointerEvents: 'none' // Ensure clicks pass through
            }}
          >
            <KeyIcon size={24} /> {/* Render the KeyIcon component */}
  ...
  // Function to toggle full-screen stats
    ...
    <div className="flex flex-col items-center justify-center w-full h-screen bg-gray-900 text-white overflow-hidden relative">
      ...
      {isStatsFullscreen ? (
        ...
        >
          ...
          <div className="absolute top-0 left-0 w-full p-2 flex justify-between items-center bg-black bg-opacity-60 shadow-lg z-30"> {/* Increased z-index, reduced padding, added opacity */}
            ...
            {!isStatsFullscreen && (
                <div className="flex items-center space-x-1 currency-display-container relative"> {/* Reduced space-x */}
                    {/* Gems Container */}
                    {/* Updated background and border for gem counter */}
                    ...
                    <div className="bg-gradient-to-br from-yellow-500 to-amber-700 rounded-lg p-0.5 flex items-center shadow-lg border border-amber-300 relative overflow-hidden group hover:scale-105 transition-all duration-300 cursor-pointer"> {/* Reduced padding */}
                     ...
                     {/* NEW: Keys Container (Next to chest count) */}
                    <div className="flex items-center space-x-1"> {/* Space between key icon and count */}
                        {/* Key Icon */}
                        <KeyIcon size={16} className="text-yellow-400"/> {/* Adjust size and color as needed */}
                         {/* Key Count */}
                        <span className="text-white text-xs font-bold">
                            {keysCollected} {/* Display collected keys */}
                        </span>
                    </div>
                </div>
             )}
          ...
          {/* Render the TreasureChest component */}
