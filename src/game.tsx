// --- START OF FILE game.tsx (Full, Integrated, Un-collapsed Version) ---
...
const groupVideosByCategory = (videos: Video[]): Record<string, Video[]> => {
    ...
    const TabButton = ({ isActive, onClick, label, count }: { isActive: boolean, onClick: () => void, label: string, count: number }) => (
        <button
            ...
            className={`flex-1 text-center px-3 py-1.5 rounded-md text-sm font-medium transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 ${
                isActive ? 'bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-200/70 dark:hover:bg-gray-600/50'
            }`}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
            ...
            >
                <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <button onClick={onClose} className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700" aria-label="Đóng">
                        <XIcon />
                    </button>
                </div>
...
const EbookReaderAndYoutubePlayer: React.FC<EbookReaderProps> = ({ hideNavBar, showNavBar }) => {
    ...
    const fetchSubtitles = async () => {
      ...
      try {
        ...
        const formattedSubtitles = data.map((sub: any) => ({
          start: parseFloat(sub.start), dur: parseFloat(sub.dur), text: sub.text,
        }));
        ...
        {currentBook && (
          <div className="mb-6 sm:mb-8 pb-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <div className="mt-6 flex flex-wrap justify-center items-center gap-4">
              ...
              <button onClick={() => setIsStatsModalOpen(true)} className="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white dark:text-gray-200 dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
  ...
  const renderLibrary = (type: 'books' | 'videos') => {
    if (type === 'books') {
              ...
              <div className="flex overflow-x-auto space-x-4 pb-4 -mx-4 px-4">
                {booksInCategory.map(book => (
                  <div key={book.id} className="flex-shrink-0 w-40 cursor-pointer group" onClick={() => handleSelectBook(book.id)}>
                    <div className="aspect-[2/3] bg-gray-200 dark:bg-gray-700 rounded-lg shadow-lg mb-2 group-hover:shadow-xl"><img src={book.coverImageUrl} alt={book.title} className="w-full h-full object-cover rounded-lg" /></div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {videosInCategory.map(video => (
                  <div key={video.id} className="cursor-pointer group" onClick={() => handleSelectVideo(video.id)}>
                    <div className="aspect-video bg-gray-200 dark:bg-gray-700 rounded-lg shadow-lg mb-2 group-hover:shadow-xl"><img src={video.thumbnailUrl} alt={video.title} className="w-full h-full object-cover rounded-lg" /></div>
  ...
  const renderVideoPlayer = () => {
    ...
    return (
      <main className="flex-grow overflow-y-auto w-full bg-gray-50 dark:bg-gray-900 py-6 sm:py-8">
        <div className="max-w-4xl mx-auto flex flex-col h-full px-4">
