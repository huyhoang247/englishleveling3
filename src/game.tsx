...
// --- Icons ---
// ... (Tất cả các icon components giữ nguyên)
const PlayIcon = () => (
...
// --- Interfaces ---
// ... (Tất cả các interfaces giữ nguyên)
...
// START: NEW INTERFACE FOR HIDDEN WORD STATE
interface HiddenWord {
    key: string; // "lineIndex-partIndex"
    word: string; // The actual word
    status: 'hidden' | 'revealed' | 'incorrect'; // State of the word
}
// END: NEW INTERFACE

// ... (Các components VoiceStepper, BookSidebar, BookStatsModal giữ nguyên)
...
const EbookReader: React.FC<EbookReaderProps> = ({ hideNavBar, showNavBar }) => {
  ...
  // START: NEW STATES FOR HIDE-AND-GUESS GAME
  const [isHidingWordsMode, setIsHidingWordsMode] = useState(false);
  ...
  const [activeHiddenWordKey, setActiveHiddenWordKey] = useState<string | null>(null);
  const [currentUserGuess, setCurrentUserGuess] = useState('');
  const [incorrectGuessKey, setIncorrectGuessKey] = useState<string | null>(null);
  // END: NEW STATES

  const currentBook = booksData.find(book => book.id === selectedBookId);

  // ... (Tất cả các useEffect và useMemo cũ giữ nguyên)
    useEffect(() => {
  ...
  useEffect(() => {
    if (selectedBookId) {
      ...
      setIsAudioPlaying(false);
      // START: RESET GAME STATE ON EXITING BOOK
      setIsHidingWordsMode(false);
      setHiddenWords(new Map());
      setActiveHiddenWordKey(null);
  ...
  // START: NEW LOGIC FOR GUESSING GAME
  const activeHiddenWord = activeHiddenWordKey ? hiddenWords.get(activeHiddenWordKey) : null;

  useEffect(() => {
    if (!activeHiddenWord || currentUserGuess.length !== activeHiddenWord.word.length) {
      return;
    }

    const checkGuess = () => {
        if (currentUserGuess.toLowerCase() === activeHiddenWord.word.toLowerCase()) {
            // Correct guess
            const newHiddenWords = new Map(hiddenWords);
            newHiddenWords.set(activeHiddenWord.key, { ...activeHiddenWord, status: 'revealed' });
            setHiddenWords(newHiddenWords);
            setActiveHiddenWordKey(null); // Close keyboard
            setCurrentUserGuess('');
        } else {
            // Incorrect guess
            setIncorrectGuessKey(activeHiddenWord.key);
            setTimeout(() => {
                setIncorrectGuessKey(null);
                setCurrentUserGuess(''); // Reset input
            }, 500); // Duration should match animation
        }
    };
    
    // Add a small delay to let user see the last typed letter
    const timer = setTimeout(checkGuess, 200);
    return () => clearTimeout(timer);

  }, [currentUserGuess, activeHiddenWord, hiddenWords]);


  const startHidingWords = () => {
    if (!currentBook) return;
    
    const allVocabOccurrences: { key: string, word: string }[] = [];
    ...
    contentLines.forEach((line, lineIndex) => {
        ...
        parts.forEach((part, partIndex) => {
            ...
            if (isWord && vocabMap.has(part.toLowerCase())) {
                allVocabOccurrences.push({ key: `${lineIndex}-${partIndex}`, word: part });
            }
        });
    });

    // Shuffle and pick words to hide
    const shuffled = allVocabOccurrences.sort(() => 0.5 - Math.random());
    const selected = shuffled.slice(0, wordsToHideCount);

    const newHiddenWords = new Map<string, HiddenWord>();
    selected.forEach(occurrence => {
        newHiddenWords.set(occurrence.key, { ...occurrence, status: 'hidden' });
    });
    setHiddenWords(newHiddenWords);
  };
  
  const handleToggleHidingMode = (enabled: boolean) => {
    setIsHidingWordsMode(enabled);
    if (enabled) {
        startHidingWords();
    } else {
        setHiddenWords(new Map());
  ...
  const handleChangeHideCount = (amount: number) => {
      setWordsToHideCount(prev => {
          const newCount = Math.max(5, Math.min(100, prev + amount));
          return newCount;
      });
  };
  
  // Reset game when count changes while active
  useEffect(() => {
      if (isHidingWordsMode) {
          startHidingWords();
      }
  }, [wordsToHideCount, isHidingWordsMode]);

  // END: NEW LOGIC FOR GUESSING GAME

  // ... (Tất cả các handlers cũ giữ nguyên)
  ...
  const renderBookContent = () => {
    // ... (logic phân tích content giữ nguyên)
    ...
    return (
      <div className="font-['Inter',_sans-serif] text-gray-800 dark:text-gray-200 px-2 sm:px-4 pb-24">
        {contentLines.map((line, lineIndex) => {
          ...
          // START: MODIFIED RENDER LOGIC FOR GAME MODE
          if (isHidingWordsMode) {
              const parts = line.split(/(\b\w+\b|[.,!?;:()'"\s`‘’“”])/g);
              renderableParts = parts.map((part, partIndex) => {
                  if (!part) return null;
                  const key = `${lineIndex}-${partIndex}`;
                  const hiddenWordData = hiddenWords.get(key);

                  if (hiddenWordData) {
                      if (hiddenWordData.status === 'revealed') {
                           return (
                              <span key={key} className="font-semibold text-green-500 animate-reveal">
                                {part}
                              </span>
                           );
                      }
                      // Word is hidden or incorrect
                      const isShaking = incorrectGuessKey === key;
                      return (
                          <button
                            key={key}
                            onClick={() => {
                                setCurrentUserGuess('');
                                setActiveHiddenWordKey(key);
                            }}
                            className={`inline-block align-baseline font-mono tracking-widest px-1 rounded-md cursor-pointer transition-all duration-200 border-b-2 
                                ${isShaking ? 'animate-shake border-red-500 bg-red-100 dark:bg-red-500/20' : 'border-dashed border-gray-400 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-700'}`}
                            style={{ minWidth: `${part.length * 0.8}em` }}
                            aria-label={`Từ bị ẩn, dài ${part.length} ký tự`}
                          >
                           {'\u00A0'.repeat(part.length)}
                          </button>
                      );
                  }
                  // Not a target word, render as text
                  return <span key={`${lineIndex}-${partIndex}`}>{part}</span>;
              }).filter(Boolean) as JSX.Element[];

          } else if (highlightMode === 'phrase' && phraseRegex) { // Logic cũ
          ...
          } else { // Logic cũ cho 'word' mode
              const parts = line.split(/(\b\w+\b|[.,!?;:()'"\s`‘’“”])/g);
              renderableParts = parts.map((part, partIndex) => {
                if (!part) return null;
                const isWord = /^\w+$/.test(part);
                const isVocabWord = isWord && vocabMap.has(part.toLowerCase());
                if (isVocabWord) {
                  return (
                    <span
                      key={`${lineIndex}-${partIndex}`}
  ...
  // ... (Các hàm togglePlayPause, handleTimeUpdate, etc. giữ nguyên)
  ...
  return (
    <div className={`flex flex-col h-screen ${isDarkMode ? 'dark' : ''} bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white`}>
      {/* ... (Header, Sidebar, Main content wrapper giữ nguyên) ... */}
      {selectedBookId && (
          ...
          <div className="max-w-2xl lg:max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 sm:p-8 md:p-10 relative">
            {currentBook && (
              <div className="mb-6 sm:mb-8 pb-4 border-b border-gray-200 dark:border-gray-700">
                ...
                {/* START: NEW GAME MODE CONTROLS */}
                <div className="mt-6 p-4 bg-gray-100 dark:bg-gray-700/50 rounded-lg">
                    <div className="flex items-center justify-between">
                        <span className="font-semibold text-gray-700 dark:text-gray-300">Chế độ Đoán từ</span>
                        <button
                            onClick={() => handleToggleHidingMode(!isHidingWordsMode)}
                            className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${ isHidingWordsMode ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600' }`}
                        >
                            <span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform ${ isHidingWordsMode ? 'translate-x-6' : 'translate-x-1' }`}/>
                        </button>
                    </div>
                    {isHidingWordsMode && (
                        <div className="mt-4 flex items-center justify-center gap-4">
                            <span className="text-sm text-gray-600 dark:text-gray-400">Số từ ẩn:</span>
                            <div className="flex items-center gap-2">
                                <button onClick={() => handleChangeHideCount(-5)} className="w-7 h-7 rounded-full bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 font-bold">-</button>
                                <span className="text-lg font-bold w-12 text-center">{wordsToHideCount}</span>
                                <button onClick={() => handleChangeHideCount(5)} className="w-7 h-7 rounded-full bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 font-bold">+</button>
                            </div>
                        </div>
                    )}
                </div>
                {/* END: NEW GAME MODE CONTROLS */}
      ...
      {/* ... (Audio player and modals cũ giữ nguyên) ... */}
       <audio
        ref={audioPlayerRef}
      ...
      {/* START: VIRTUAL KEYBOARD UI */}
      {activeHiddenWordKey && activeHiddenWord && (
          <>
            <div 
                className="fixed inset-0 bg-black/50 z-40" 
                onClick={() => setActiveHiddenWordKey(null)}
            />
            <div className="fixed bottom-0 left-0 right-0 z-50 flex flex-col items-center pb-4 pt-2 bg-gray-200 dark:bg-gray-800 rounded-t-xl shadow-2xl">
                <div className="w-full text-center mb-2 px-4">
                    <p className="text-sm text-gray-600 dark:text-gray-400">Đoán từ có {activeHiddenWord.word.length} chữ cái</p>
                    {/* Visual representation of the input */}
                    <div className="flex justify-center gap-1 mt-1 h-10">
                        {Array.from({ length: activeHiddenWord.word.length }).map((_, i) => (
                            <div key={i} className="w-8 h-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md flex items-center justify-center text-xl font-bold">
                                {currentUserGuess[i]?.toUpperCase() || ''}
                            </div>
                        ))}
                    </div>
                </div>
                <VirtualKeyboard
                    userInput={currentUserGuess}
                    setUserInput={setCurrentUserGuess}
                    wordLength={activeHiddenWord.word.length}
                    disabled={!!incorrectGuessKey} // Disable keyboard during wrong animation
                />
            </div>
          </>
      )}
