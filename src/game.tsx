// --- START OF FILE src/game.tsx (PHIÊN BẢN ĐẦY ĐỦ VÀ HOÀN CHỈNH) ---

import React, { useState, useEffect, useRef, useMemo } from 'react';
import YouTube from 'react-youtube';
// Giả sử các file này tồn tại trong dự án của bạn
...
// --- Icons ---
function PlayIcon() { return ( <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path fillRule="evenodd" d="M4.5 5.653c0-1.427 1.529-2.33 2.779-1.643l11.54 6.347c1.295.712 1.295 2.573 0 3.286L7.28 19.99c-1.25.687-2.779-.217-2.779-1.643V5.653Z" clipRule="evenodd" /></svg> ); }
function PauseIcon() { return ( <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path fillRule="evenodd" d="M6.75 5.25a.75.75 0 0 1 .75.75V18a.75.75 0 0 1-1.5 0V6a.75.75 0 0 1 .75-.75Zm9 0a.75.75 0 0 1 .75.75V18a.75.75 0 0 1-1.5 0V6a.75.75 0 0 1 .75-.75Z" clipRule="evenodd" /></svg> ); }
function MenuIcon() { return ( <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" /></svg> ); }
function XIcon() { return ( <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg> ); }
function StatsIcon() { return ( <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5 3a1 1 0 00-1 1v12a1 1 0 102 0V4a1 1 0 00-1-1zM9 9a1 1 0 00-1 1v6a1 1 0 102 0v-6a1 1 0 00-1-1zm4-5a1 1 0 00-1 1v10a1 1 0 102 0V5a1 1 0 00-1-1z" clipRule="evenodd" /></svg> ); }

// --- Interfaces ---
interface Video { id: string; title: string; author: string; thumbnailUrl: string; category: string; }
...
// --- Utility Functions ---
function groupBooksByCategory(books: Book[]): Record<string, Book[]> { return books.reduce((acc, book) => { const category = book.category || 'Uncategorized'; if (!acc[category]) { acc[category] = []; } acc[category].push(book); return acc; }, {} as Record<string, Book[]>); }
function groupVideosByCategory(videos: Video[]): Record<string, Video[]> { return videos.reduce((acc, video) => { const category = video.category || 'Uncategorized'; if (!acc[category]) { acc[category] = []; } acc[category].push(video); return acc; }, {} as Record<string, Video[]>); }

// --- Components (BookSidebar, BookStatsModal) ---
// Lưu ý: Dán code đầy đủ của các component này của bạn vào đây nếu có
function BookSidebar({ isOpen, onClose, book, isDarkMode, toggleDarkMode }: any) {
  useEffect(() => { const handleEsc = (event: KeyboardEvent) => { if (event.key === 'Escape') { onClose(); } }; if (isOpen) { document.addEventListener('keydown', handleEsc); } return () => { document.removeEventListener('keydown', handleEsc); }; }, [isOpen, onClose]);
...
export default function EbookReaderAndYoutubePlayer({ hideNavBar, showNavBar }: EbookReaderProps) {
  ...
  useEffect(() => { const tempMap = new Map<string, Vocabulary>(); defaultVocabulary.forEach((word) => { tempMap.set(word.toLowerCase(), { word, meaning: `Nghĩa của từ "${word}".`, example: `Ví dụ cho từ "${word}".`, phrases: [`Cụm từ với ${word}`], popularity: "Cao", synonyms: [`Đồng nghĩa ${word}`], antonyms: [`Trái nghĩa ${word}`] }); }); setVocabMap(tempMap); setIsLoadingVocab(false); }, []);
  useEffect(() => { document.documentElement.classList.toggle('dark', isDarkMode); }, [isDarkMode]);
  useEffect(() => { const isReadingOrWatching = selectedBookId || selectedVideoId; if (isReadingOrWatching) { hideNavBar(); } else { showNavBar(); } return () => { if (audioPlayerRef.current) audioPlayerRef.current.pause(); if (intervalRef.current) clearInterval(intervalRef.current); }; }, [selectedBookId, selectedVideoId, hideNavBar, showNavBar]);
  
  // === TẢI DỮ LIỆU VIDEO TỪ FILE JSON (MỚI) ===
  useEffect(() => {
  ...
  // === TẢI PHỤ ĐỀ TỪ FILE JSON (ĐÃ SỬA) ===
  useEffect(() => {
    if (viewMode !== 'videos' || !selectedVideoId) return;
    const fetchSubtitlesFromFile = async () => {
      setIsLoadingSubtitles(true); setSubtitles([]); setCurrentSubtitle(null);
      try {
        const response = await fetch(`/captions/${selectedVideoId}.json`);
        if (!response.ok) throw new Error('Không tìm thấy file phụ đề.');
        const data = await response.json();
        // Script đã định dạng dữ liệu, giờ chỉ cần chuyển đổi lại kiểu cho an toàn
        ...
        console.error("Lỗi tải file phụ đề:", error); 
        setSubtitles([]);
      } 
      finally { setIsLoadingSubtitles(false); }
    };
    fetchSubtitlesFromFile();
  }, [selectedVideoId, viewMode]);

  const currentBook = useMemo(() => booksData.find(book => book.id === selectedBookId), [booksData, selectedBookId]);
  useEffect(() => { if (viewMode === 'books' && selectedBookId && currentBook?.audioUrl && audioPlayerRef.current) { audioPlayerRef.current.src = currentBook.audioUrl; audioPlayerRef.current.playbackRate = playbackSpeed; } }, [selectedBookId, viewMode, playbackSpeed, currentBook]);
  ...
  function renderVideoPlayer() { const currentVideo = videosData.find(v => v.id === selectedVideoId); return ( <main className="flex-grow overflow-y-auto w-full bg-gray-50 dark:bg-gray-900 py-6 sm:py-8"><div className="max-w-4xl mx-auto flex flex-col h-full px-4">{currentVideo && <h1 className="text-2xl font-bold text-center mb-4">{currentVideo.title}</h1>}<div className="w-full aspect-video bg-black rounded-lg overflow-hidden shadow-2xl flex-shrink-0"><YouTube videoId={selectedVideoId!} opts={{ width: '100%', height: '100%', playerVars: { origin: 'https://englishleveling3.vercel.app' } }} onReady={onPlayerReady} onStateChange={onPlayerStateChange} className="w-full h-full"/></div><div className="mt-4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg text-center text-xl md:text-2xl font-semibold min-h-[8rem] flex items-center justify-center shadow-inner flex-grow">{renderInteractiveSubtitle()}</div></div></main> ); }
...
// --- END OF FILE src/game.tsx (FULL AND COMPLETE VERSION) ---
