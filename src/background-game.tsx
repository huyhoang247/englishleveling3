// --- START OF FILE src/background-game.tsx (UPDATED) ---
...
import EquipmentScreen, { OwnedItem, EquippedItems } from './equipment.tsx'; // <-- Interface vẫn được import để dùng ở nơi khác
...
import { 
  ...
  // updateUserInventory, // <-- Không còn cần trực tiếp ở đây
  processShopPurchase
...
export default function ObstacleRunnerGame({ className, hideNavBar, showNavBar, currentUser, assetsLoaded }: ObstacleRunnerGameProps) {
  ...
  // THAY ĐỔI: Không cần quản lý state equipment chi tiết ở đây nữa
  ...
  // Các useEffect và hàm xử lý ban đầu (KHÔNG THAY ĐỔI)
  useEffect(() => { const originalHtmlOverflow = document.documentElement.style.overflow; const originalBodyOverflow = document.body.style.overflow; document.documentElement.style.overflow = 'hidden'; document.body.style.overflow = 'hidden'; const setAppHeight = () => { document.documentElement.style.setProperty('--app-height', `${window.innerHeight}px`); }; window.addEventListener('resize', setAppHeight); setAppHeight(); return () => { document.documentElement.style.overflow = originalHtmlOverflow; document.body.style.overflow = originalBodyOverflow; window.removeEventListener('resize', setAppHeight); }; }, []);
  ...
  const handleBossFloorUpdate = async (newFloor: number) => { const userId = auth.currentUser?.uid; if (!userId) { console.error("Cannot update boss floor: User not authenticated."); return; } try { await updateUserBossFloor(userId, newFloor, bossBattleHighestFloor); if (newFloor > bossBattleHighestFloor) { setBossBattleHighestFloor(newFloor); } console.log(`Updated boss floor to ${newFloor} for user ${userId} via service.`); } catch (error) { console.error("Firestore update failed for boss floor via service: ", error); } };
  const handleMinerChallengeEnd = async (result: { finalPickaxes: number; coinsEarned: number; highestFloorCompleted: number; }) => { const userId = auth.currentUser?.uid; if (!userId) { console.error("Cannot update game data: User not authenticated."); return; } if (result.finalPickaxes === pickaxes && result.coinsEarned === 0 && result.highestFloorCompleted <= minerChallengeHighestFloor) { console.log("No changes to update from Miner Challenge."); return; } setIsSyncingData(true); try { const { newCoins, newPickaxes, newHighestFloor } = await processMinerChallengeResult(userId, result); setCoins(newCoins); setPickaxes(newPickaxes); setMinerChallengeHighestFloor(newHighestFloor); console.log("Firestore updated successfully after Miner Challenge via service."); } catch (error) { console.error("Service call for Miner Challenge end failed: ", error); } finally { setIsSyncingData(false); } };
  const handleUpdatePickaxes = async (amountToAdd: number) => { const userId = auth.currentUser?.uid; if (!userId) return; const newTotal = await updateUserPickaxes(userId, pickaxes + amountToAdd); setPickaxes(newTotal); };
  const handleUpdateJackpotPool = async (amount: number, reset: boolean = false) => { const newPool = await updateJackpotPool(amount, reset); setJackpotPool(newPool); };
  const handleStatsUpdate = (newCoins: number, newStats: { hp: number; atk: number; def: number; }) => { setCoins(newCoins); setUserStats(newStats); console.log("Main game state updated from UpgradeStatsScreen."); };

  // THAY ĐỔI: Xóa handleInventoryUpdate vì không còn cần thiết ở đây nữa.

  const handleShopPurchase = async (item: any, quantity: number) => { const userId = auth.currentUser?.uid; if (!userId) { throw new Error("Người dùng chưa được xác thực."); } if (!item || typeof item.price !== 'number' || !item.id || typeof quantity !== 'number' || quantity <= 0) { throw new Error("Dữ liệu vật phẩm hoặc số lượng không hợp lệ."); } setIsSyncingData(true); try { const { newCoins, newBooks, newCapacity } = await processShopPurchase(userId, item, quantity); setCoins(newCoins); if (item.id === 1009) { setAncientBooks(newBooks); } else if (item.id === 2001) { setCardCapacity(newCapacity); } console.log(`Purchase successful for ${quantity}x ${item.name}.`); alert(`Mua thành công x${quantity} ${item.name}!`); } catch (error) { console.error("Shop purchase transaction failed:", error); alert(`Mua thất bại: ${error instanceof Error ? error.message : String(error)}`); throw error; } finally { setIsSyncingData(false); } };
  ...
  const refreshUserData = useCallback(async () => {
    ...
    try {
      ...
      // THAY ĐỔI: Không cần set state equipment chi tiết ở đây nữa
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      if (user) {
        setIsLoadingUserData(true);
        try { const [_, jackpotData] = await Promise.all([ refreshUserData(), fetchJackpotPool() ]); setJackpotPool(jackpotData); } catch (error) { console.error("Error fetching initial user/app data:", error); setIsLoadingUserData(false); }
      } else {
        ...
        // THAY ĐỔI: không cần reset state equipment
        // setEquipmentPieces(0); setOwnedItems([]); setEquippedItems({ weapon: null, armor: null, accessory: null }); 
  ...
  const renderCharacter = () => { const isAnyOverlayOpen = isRankOpen || isPvpArenaOpen || isLuckyGameOpen || isBossBattleOpen || isShopOpen || isVocabularyChestOpen || isAchievementsOpen || isAdminPanelOpen || isMinerChallengeOpen || isUpgradeScreenOpen; const isPaused = isAnyOverlayOpen || isLoading || isBackgroundPaused; return (<div className="character-container absolute w-28 h-28 left-1/2 -translate-x-1/2 bottom-40 z-20"><DotLottieReact src={lottieAssets.characterRun} loop autoplay={!isPaused} className="w-full h-full" /></div>); };
  const createToggleFunction = (setter: React.Dispatch<React.SetStateAction<boolean>>) => { return () => { if (isLoading) return; if (isSyncingData) { setShowRateLimitToast(true); return; } setter(prev => { const newState = !prev; if (newState) { hideNavBar(); [ setIsRankOpen, setIsPvpArenaOpen, setIsLuckyGameOpen, setIsMinerChallengeOpen, setIsBossBattleOpen, setIsShopOpen, setIsVocabularyChestOpen, setIsSkillScreenOpen, setIsEquipmentOpen, setIsAchievementsOpen, setIsAdminPanelOpen, setIsUpgradeScreenOpen, setIsBaseBuildingOpen ].forEach(s => { if (s !== setter) s(false); }); } else { showNavBar(); } return newState; }); }; };
  ...
  const handleSkillScreenClose = (dataUpdated: boolean) => { toggleSkillScreen(); if (dataUpdated) { console.log("Skill data changed. Refreshing main game data..."); refreshUserData(); } };
  ...
  const getPlayerBattleStats = () => {
      // THAY ĐỔI: Cần lấy dữ liệu equipment mới nhất. Cách đơn giản là giả định state không đổi khi modal đang mở,
      // hoặc cách phức tạp hơn là phải fetch lại. Tạm thời giữ logic cũ vì user data chỉ refresh khi modal đóng.
      // Để chính xác hơn, hàm này cần được truyền owned/equipped items vào. Nhưng vì nó chỉ dùng khi mở modal mới,
      // nên dữ liệu hiện tại trong state `background-game` là đủ chính xác.
      ...
      // Tạm thời, chúng ta cần fetch lại hoặc giả định không có thay đổi. 
      // Để đơn giản, giả sử không thay đổi. Khi các modal khác cũng được refactor,
      // logic này sẽ cần được xem xét lại.
      // Dữ liệu equipment không còn trong state này nữa, nên ta phải tạm thời bỏ qua bonus từ item.
      // ĐÂY LÀ MỘT VẤN ĐỀ CẦN GIẢI QUYẾT SAU.
  ...
  const getEquippedSkillsDetails = () => { if (!ownedSkills || !equippedSkillIds) return []; return equippedSkillIds.map(equippedId => { if (!equippedId) return null; const owned = ownedSkills.find(s => s.id === equippedId); if (!owned) return null; const blueprint = ALL_SKILLS.find(b => b.id === owned.skillId); if (!blueprint) return null; return { ...owned, ...blueprint }; }).filter((skill): skill is OwnedSkill & SkillBlueprint => skill !== null); };
  ...
  // THÊM MỚI: Callback để EquipmentScreen thông báo khi dữ liệu thay đổi
  const handleEquipmentDataUpdated = useCallback(() => {
    console.log("Equipment data changed. Refreshing main game data to update header...");
    refreshUserData(); // Chỉ cần refresh là đủ để cập nhật coin, gem... trên header
  }, [refreshUserData]);
  ...
  return (
    <div className="w-screen h-[var(--app-height)] overflow-hidden bg-gray-950 relative">
      <SidebarLayout setToggleSidebar={handleSetToggleSidebar} onShowRank={toggleRank} onShowLuckyGame={toggleLuckyGame} onShowMinerChallenge={toggleMinerChallenge} onShowAchievements={toggleAchievements} onShowUpgrade={toggleUpgradeScreen} onShowBaseBuilding={toggleBaseBuilding} onShowAdmin={isAdmin ? toggleAdminPanel : undefined}>
        ...
        <div className="fixed inset-0 z-[60]" style={{ display: isAchievementsOpen ? 'block' : 'none' }}> <ErrorBoundary>{isAchievementsOpen && (<AchievementsScreen user={auth.currentUser} onClose={toggleAchievements} onDataUpdate={handleAchievementsDataUpdate} />)}</ErrorBoundary> </div>
        ...
        {/* THAY ĐỔI LỚN: Cách gọi EquipmentScreen đã được đơn giản hóa */}
            ...
            <ErrorBoundary>
                {isEquipmentOpen && auth.currentUser && (
                    <EquipmentScreen 
                        ...
                        onDataUpdated={handleEquipmentDataUpdated} 
                    />
                )}
            </ErrorBoundary>
        </div>
