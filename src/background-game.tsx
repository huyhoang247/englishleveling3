...
const XIcon = ({ size = 24, color = 'currentColor', className = '', ...props }) => (
  <svg
    ...
    className={`lucide-icon ${className}`} {...props} >
...
const calculateLevelAndExp = (totalExp: number): { level: number; exp: number; maxExp: number } => {
  if (totalExp <= 0) {
    return { level: 1, exp: 0, maxExp: 100 };
  }
  let level = 1;
  let expForNextLevel = 100;
  let cumulativeExp = 0;
  while (totalExp >= cumulativeExp + expForNextLevel) {
    cumulativeExp += expForNextLevel;
    level++;
    expForNextLevel = level * 100;
  }
  const expInCurrentLevel = totalExp - cumulativeExp;
  return {
    level: level,
...
export default function ObstacleRunnerGame({ className, hideNavBar, showNavBar, currentUser, assetsLoaded }: ObstacleRunnerGameProps) {
  ...
  const fetchAndProcessAchievementsData = async (userId: string) => {
    try {
      const completedWordsRef = collection(db, 'users', userId, 'completedWords');
      const querySnapshot = await getDocs(completedWordsRef);
      
      const processedData: VocabularyItem[] = [];
      let docIndex = 0; 

      querySnapshot.forEach((docSnap) => {
        const word = docSnap.id;
        const data = docSnap.data();
        
        let totalCorrectCount = 0;
        if (data.gameModes && typeof data.gameModes === 'object') {
          totalCorrectCount = Object.values(data.gameModes).reduce((sum: number, mode: any) => {
              return sum + (mode.correctCount || 0);
          }, 0);
        }

        const totalExp = totalCorrectCount * 100;
        const { level, exp, maxExp } = calculateLevelAndExp(totalExp);
      ...
      console.log("Processed vocabulary achievements data:", processedData);
      setVocabularyData(processedData);
    } catch (error) {
      console.error("Error fetching and processing achievements data:", error);
      setVocabularyData([]);
    }
  };
  ...
  const fetchUserData = async (userId: string) => {
    ...
    try {
      ...
      if (userDocSnap.exists()) {
        ...
        await setDoc(userDocRef, { coins: 0, gems: 0, createdAt: new Date() });
        setCoins(0); setDisplayedCoins(0); setGems(0);
      }
    } catch (error) {
  ...
  const startCoinCountAnimation = (reward: number) => {
    ...
    let step = Math.ceil(reward / 30) || 1;
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(user => {
      if (user) {
        ...
        fetchAndProcessAchievementsData(user.uid);
        fetchJackpotPool();
      } else {
        setIsLoadingUserData(true);
        setVocabularyData(null);
        setCoins(0); setDisplayedCoins(0); setGems(0); setJackpotPool(0);
        setIsStatsFullscreen(false); setIsRankOpen(false); setIsGoldMineOpen(false); setIsInventoryOpen(false);
        setIsLuckyGameOpen(false); setIsBlacksmithOpen(false); setIsTowerGameOpen(false); setIsShopOpen(false);
        setIsVocabularyChestOpen(false); setIsAchievementsOpen(false); setIsAdminPanelOpen(false);
  ...
  const isLoading = isLoadingUserData || vocabularyData === null || !assetsLoaded;

  useEffect(() => {
    if (Math.abs(coins - displayedCoins) > 100) {
        setDisplayedCoins(coins);
    }
  }, [coins]);

  const createToggleHandler = (setter, requiresAuth = false) => () => {
    if (isLoading || (requiresAuth && !auth.currentUser)) return;
  ...
  return (
    <div className="w-screen h-[var(--app-height)] overflow-hidden bg-gray-950 relative">
      <SidebarLayout
          ...
          onShowStats={createToggleHandler(setIsStatsFullscreen, true)}
          onShowRank={createToggleHandler(setIsRankOpen, true)}
          onShowGoldMine={createToggleHandler(setIsGoldMineOpen, true)}
          onShowLuckyGame={createToggleHandler(setIsLuckyGameOpen, true)}
          onShowAchievements={createToggleHandler(setIsAchievementsOpen, true)}
          onShowAdmin={isAdmin ? createToggleHandler(setIsAdminPanelOpen) : undefined}
      >
        <DungeonCanvasBackground isPaused={isGamePaused} />
        
        <div style={{ display: isAnyOverlayOpen ? 'block' : 'none', visibility: isLoading ? 'hidden' : 'visible' }} className="w-full h-full">
          <div className={`${className ?? ''} relative w-full h-full rounded-lg overflow-hidden shadow-2xl bg-transparent`} onClick={handleTap}>
            ...
            <div className="absolute left-4 bottom-32 flex flex-col space-y-4 z-30">
              {[
                { icon: <img src={uiAssets.towerIcon} alt="Leo Tháp Icon" className="w-full h-full object-contain" />, onClick: createToggleHandler(setIsTowerGameOpen) },
                { icon: <img src={uiAssets.shopIcon} alt="Shop Icon" className="w-full h-full object-contain" />, onClick: createToggleHandler(setIsShopOpen) },
                { icon: <img src={uiAssets.inventoryIcon} alt="Inventory Icon" className="w-full h-full object-contain" />, onClick: createToggleHandler(setIsInventoryOpen, true) }
            ...
            <div className="absolute right-4 bottom-32 flex flex-col space-y-4 z-30">
              {[
                { icon: <img src="https://raw.githubusercontent.com/huyhoang247/englishleveling3/refs/heads/main/src/icon/file_00000000fe00622fb8cc4792a683dcb3.png" alt="Vocabulary Chest Icon" className="w-full h-full object-contain" />, onClick: createToggleHandler(setIsVocabularyChestOpen, true) },
                { icon: <img src={uiAssets.missionIcon} alt="Mission Icon" className="w-full h-full object-contain" /> },
                { icon: <img src={uiAssets.blacksmithIcon} alt="Blacksmith Icon" className="w-full h-full object-contain" />, onClick: createToggleHandler(setIsBlacksmithOpen, true) },
              ].map((item, index) => (<div key={index} className="group cursor-pointer"><div className="scale-105 relative transition-all duration-300 flex flex-col items-center justify-center w-14 h-14 flex-shrink-0 bg-black bg-opacity-20 p-1.5 rounded-lg" onClick={item.onClick}>{item.icon}</div></div>))}
            </div>
          </div>
        </div>
        
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isStatsFullscreen ? 'block' : 'none' }}><ErrorBoundary>{auth.currentUser && (<CharacterCard onClose={createToggleHandler(setIsStatsFullscreen)} coins={coins} onUpdateCoins={(amount) => updateCoinsInFirestore(auth.currentUser!.uid, amount)}/>)}</ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isRankOpen ? 'block' : 'none' }}><ErrorBoundary><EnhancedLeaderboard onClose={createToggleHandler(setIsRankOpen)} /></ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isGoldMineOpen ? 'block' : 'none' }}><ErrorBoundary>{auth.currentUser && (<GoldMine onClose={createToggleHandler(setIsGoldMineOpen)} currentCoins={coins} onUpdateCoins={(amount) => updateCoinsInFirestore(auth.currentUser!.uid, amount)} onUpdateDisplayedCoins={(amount) => setDisplayedCoins(amount)} currentUserId={auth.currentUser!.uid} isGamePaused={isAnyOverlayOpen}/>)}</ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isInventoryOpen ? 'block' : 'none' }}><ErrorBoundary><Inventory onClose={createToggleHandler(setIsInventoryOpen)} /></ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isLuckyGameOpen ? 'block' : 'none' }}><ErrorBoundary>{auth.currentUser && (<LuckyChestGame onClose={createToggleHandler(setIsLuckyGameOpen)} currentCoins={coins} onUpdateCoins={(amount) => updateCoinsInFirestore(auth.currentUser!.uid, amount)} currentJackpotPool={jackpotPool} onUpdateJackpotPool={(amount, reset) => updateJackpotPoolInFirestore(amount, reset)} isStatsFullscreen={isStatsFullscreen}/>)}</ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isBlacksmithOpen ? 'block' : 'none' }}><ErrorBoundary><Blacksmith onClose={createToggleHandler(setIsBlacksmithOpen)} /></ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isTowerGameOpen ? 'block' : 'none' }}><ErrorBoundary>{isTowerGameOpen && <TowerExplorerGame onClose={createToggleHandler(setIsTowerGameOpen)} />}</ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isShopOpen ? 'block' : 'none' }}><ErrorBoundary>{isShopOpen && <Shop onClose={createToggleHandler(setIsShopOpen)} />}</ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isVocabularyChestOpen ? 'block' : 'none' }}><ErrorBoundary>{isVocabularyChestOpen && (<VocabularyChestScreen onClose={createToggleHandler(setIsVocabularyChestOpen)} currentUserId={currentUser ? currentUser.uid : null} onCoinReward={startCoinCountAnimation} onGemReward={handleGemReward}/>)}</ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[60]" style={{ display: isAchievementsOpen ? 'block' : 'none' }}><ErrorBoundary>{isAchievementsOpen && auth.currentUser && vocabularyData && (<AchievementsScreen onClose={createToggleHandler(setIsAchievementsOpen)} userId={auth.currentUser.uid} initialData={vocabularyData}/>)}</ErrorBoundary></div>
        <div className="absolute inset-0 w-full h-full z-[70]" style={{ display: isAdminPanelOpen ? 'block' : 'none' }}><ErrorBoundary>{isAdminPanelOpen && <AdminPanel onClose={createToggleHandler(setIsAdminPanelOpen)} />}</ErrorBoundary></div>
