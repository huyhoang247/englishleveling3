...
export default function ObstacleRunnerGame({ className, hideNavBar, showNavBar, currentUser }: ObstacleRunnerGameProps) {
  ...
  // REMOVED: Initializing with 200, it will now be fetched from Firestore
  ...
  const fetchUserData = async (userId: string) => {
    ...
    try {
      ...
      if (userDocSnap.exists()) {
        ...
        // REMOVED: Fetching jackpotPool from user doc, it's now global
        ...
        await setDoc(userDocRef, {
          ...
          gems: 0,
          keys: 0,
          // REMOVED: setting jackpotPool in user doc
        ...
        setGems(0);
        setKeyCount(0);
        // REMOVED: setting jackpotPool in local state
        // setJackpotPool(200);
      }
    } catch (error) {
  ...
  const fetchJackpotPool = async () => {
    try {
        ...
        if (jackpotDocSnap.exists()) {
            ...
            setJackpotPool(data.poolAmount || 200); // Default to 200 if not set
            console.log("Jackpot Pool fetched:", data.poolAmount);
        } else {
            // If global jackpot pool document doesn't exist, create it with default value
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!userDoc.exists()) {
          ...
          transaction.set(userDocRef, {
            ...
            // REMOVED: jackpotPool from user doc
            createdAt: new Date()
          });
        } else {
      ...
      try {
          ...
          await runTransaction(db, async (transaction) => {
              ...
              if (!jackpotDoc.exists()) {
                  console.error("Global jackpot pool document does not exist, creating it.");
                  newJackpotPool = resetToDefault ? 200 : 200 + amount; // Default to 200 if not exists, then apply amount
              ...
              setJackpotPool(newJackpotPool); // Update local state after successful Firestore update
              console.log(`Jackpot Pool updated in Firestore: -> ${newJackpotPool}`);
          });
  ...
  // Effect to fetch user data and global jackpot pool from Firestore on authentication state change
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(user => {
      if (user) {
        console.log("User authenticated:", user.uid);
        fetchUserData(user.uid); // Fetch user data when authenticated
        fetchJackpotPool(); // Fetch global jackpot pool
        ...
        setJackpotPool(0); // Reset local jackpot pool
          ...
          <ErrorBoundary fallback={<div className="text-center p-4 bg-red-900 text-white rounded-lg">Lỗi hiển thị Lucky Game!</div>}>
              {auth.currentUser && (
                  <LuckyChestGame
                      ...
                      currentJackpotPool={jackpotPool} // Truyền giá trị jackpotPool hiện tại (đã được fetch từ global)
                      onUpdateJackpotPool={(amount, reset) => updateJackpotPoolInFirestore(amount, reset)} // Truyền hàm cập nhật jackpotPool global
