    ...
    <div className="w-screen h-[var(--app-height)] overflow-hidden bg-gray-950 relative">
      ...
      >
           ...
           <div className={`${className ?? ''} relative w-full h-full rounded-lg overflow-hidden shadow-2xl bg-transparent`} onClick={handleTap}>
            ...
            <div className="absolute left-4 bottom-32 flex flex-col space-y-4 z-30">
              {[ 
                  ...
                  // TRADE ASSOCIATION BUTTON (Thay thế 777)
                  { icon: <img src={TRADE_ICON_URL} alt="Trade Association" className="w-full h-full object-contain p-0.5" />, onClick: toggleTradeAssociation } 
        ...
        {isTradeAssociationOpen && (
            <ErrorBoundary fallback={<div className="fixed inset-0 bg-black/70 flex items-center justify-center text-red-400">Error Loading Trade.</div>}>
                <Suspense fallback={<SuspenseLoader />}>
                    <TradeAssociationModalV2 
                        isOpen={isTradeAssociationOpen} 
                        onClose={toggleTradeAssociation} 
                    />
                </Suspense>
            </ErrorBoundary>
        )}
