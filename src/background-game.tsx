...
// NEW: Key Icon Component (Duplicated from treasure.tsx for use in obstacle rendering)
const KeyIcon = () => (
  <img
...
export default function ObstacleRunnerGame({ className }: ObstacleRunnerGameProps) {
  ...
  const startGame = () => {
    ...
    nextKeyInRef.current = randomBetween(5, 10); // NEW: Reset key drop interval
    ...
    if (obstacleTypes.length > 0) {
        ...
        // NEW: Determine if the first obstacle has a key
        const hasKeyFirst = (() => {
        ...
        initialObstacles.push({
          ...
          hasKey: hasKeyFirst, // NEW: Add hasKey flag
        });
        ...
        for (let i = 1; i < 5; i++) {
          ...
          // NEW: Determine if this obstacle has a key
          const hasKey = (() => {
            nextKeyInRef.current -= 1;
            if (nextKeyInRef.current <= 0) {
              nextKeyInRef.current = randomBetween(5, 10);
    ...
    <div className="flex flex-col items-center justify-center w-full h-screen bg-gray-900 text-white overflow-hidden relative">
      ...
      {isStatsFullscreen ? (
        ...
        >
          ...
          <TreasureChest
            initialChests={3}
            keyCount={keyCount} // Pass the key count
            onKeyCollect={(n) => {
              // This callback is triggered by the TreasureChest when a chest is opened
              // You can add logic here to handle what happens when a key is "used" to open a chest,
              // e.g., decrease key count and potentially increase chests remaining or unlock rewards.
              // For now, based on your instructions, we'll just log it or handle chest unlock logic.
              console.log(`Chest opened using ${n} key(s).`);
              // Example: Decrease key count when a chest is successfully opened (assuming 1 key per chest)
              setKeyCount(prev => Math.max(0, prev - n)); // Ensure key count doesn't go below zero
              // Example: Unlock a chest (this logic might be more complex depending on how chests are unlocked)
              // setChestsRemaining(prev => prev + n); // This seems counter-intuitive for unlocking, maybe it means gaining a new chest?
              // A more typical flow might be: Collect keys -> Use keys to open chests -> Chests give rewards.
              // The current setup seems to imply collecting keys *from* enemies to open chests.
              // Let's assume for now that onKeyCollect from TreasureChest is meant to signal a key was *used* to open a chest.
              // The logic for *gaining* keys from enemies is handled in the collision detection.
            }}
