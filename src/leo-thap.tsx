...
const FloorSelectionScreen = ({ highestFloorCleared, onSelectFloor }) => {
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case 'completed':
        return { icon: '✅', label: 'Farm', bgColor: 'bg-green-800/50 hover:bg-green-700/70', textColor: 'text-green-300', borderColor: 'border-green-500/30' };
      case 'current':
        return { icon: '⚔️', label: 'Challenge', bgColor: 'bg-yellow-600/60 hover:bg-yellow-500/80 animate-pulse', textColor: 'text-yellow-300', borderColor: 'border-yellow-400/50' };
      default: // locked
        return { icon: '🔒', label: 'Locked', bgColor: 'bg-gray-800/50 cursor-not-allowed', textColor: 'text-gray-500', borderColor: 'border-gray-600/30' };
    }
  };

  return (
    <div className="w-full h-full flex flex-col items-center p-4 animate-fade-in">
        <h2 className="text-2xl font-bold mb-4 text-gray-200">Highest Floor: {highestFloorCleared}</h2>
        <div ref={scrollRef} className="w-full max-w-md space-y-3 overflow-y-auto pr-2">
            {floors.map(floor => {
                ...
                return (
                    <button
                        key={floor} id={`floor-${floor}`}
                        onClick={() => !isLocked && onSelectFloor(floor)} disabled={isLocked}
...
const TowerExplorerGame = ({ onClose }: TowerExplorerGameProps) => {
  ...
  const returnToMap = () => {
    if (battleFloor > highestFloorCleared) setHighestFloorCleared(battleFloor);
        ...
        {gameState !== 'floor_selection' && (
            <div className="p-4 bg-black/20 animate-fade-in">
                <div className="flex justify-between items-center text-sm md:text-base">
                    <div className="flex items-center space-x-2" title="Health"><span className="text-lg">❤️</span><span className="font-semibold">{playerStats.health}/{playerStats.maxHealth}</span></div>
