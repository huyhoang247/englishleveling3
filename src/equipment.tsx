// --- START OF FILE item-database (gff2).ts.txt ---
...
--- START OF FILE equipment.tsx (11).txt ---
                                ...
                                <div className="p-4">
                                    {activeTab === 'stats' && (
                                        <div className="space-y-1">
                                            {hasStats ? Object.entries(ownedItem.stats).map(([key, currentValue]) => {
                                                const config = STAT_CONFIG[key.toLowerCase()];
                                                // Lấy chỉ số gốc (level 1) từ định nghĩa vật phẩm
                                                const baseStat = itemDef.stats?.[key];

                                                // Kiểm tra xem có thể tính toán và hiển thị sự thay đổi chỉ số không
                                                const isNumeric = typeof currentValue === 'number' && typeof baseStat === 'number';
                                                const bonus = isNumeric ? currentValue - baseStat : 0;
                                                ...
                                                return (
                                                    <div key={key} className="flex items-center gap-2 bg-slate-900/50 p-1.5 rounded-lg">
                                                        ...
                                                        <div className="flex flex-1 items-center justify-between">
                                                            ...
                                                            {/* Giao diện hiển thị chỉ số mới */}
                                                            <div className="flex items-baseline gap-2 font-mono text-sm">
                                                                {isNumeric ? (
                                                                    <>
                                                                        <span className="text-slate-400">{baseStat.toLocaleString()}</span>
                                                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                                                                        <span className="font-bold text-white">{currentValue.toLocaleString()}</span>
                                                                        {bonus > 0 && (
                                                                    ...
                                                                    // Hiển thị cho các chỉ số không phải là số
                                                                    <span className="font-bold text-white">{String(currentValue)}</span>
                                                                )}
                                                            </div>
                                                        </div>
                                                    </div>
