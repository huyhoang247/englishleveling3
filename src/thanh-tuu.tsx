...
import type { VocabularyItem } from './vocabulary-data.ts';
...
export default function AchievementsScreen({ onClose, userId, initialData }: AchievementsScreenProps) {
  ...
  const handleClaim = useCallback(async (word: string) => {
    ...
    const totalExp = itemToClaim.totalExp;
    ...
    try {
      ...
      setVocabulary(prev => prev.map(item => {
        if (item.word === word) {
          ...
          let expRequiredForNewLevel = 0;
          for (let i = 1; i < newLevel; i++) { expRequiredForNewLevel += i * 100; }
          const newCurrentExp = totalExp - expRequiredForNewLevel;
          return { ...item, level: newLevel, exp: newCurrentExp, maxExp: newMaxExp };
        }
        return item;
      }));
    } catch (error) { console.error("Lỗi khi nhận thưởng:", error); }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-800 to-slate-900 text-white font-sans p-4 sm:p-8 flex justify-center overflow-y-auto">
      <div className="w-full max-w-4xl mx-auto relative">
        ...
        <header className="text-center mb-8"><h1 className="text-4xl sm:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-teal-300 pb-2">Thành Tựu</h1></header>
        ...
        <main className="bg-slate-900/40 p-2 sm:p-3 rounded-2xl shadow-2xl shadow-cyan-500/20 border border-slate-700">
          <div className="grid grid-cols-12 gap-4 px-4 py-3 text-sm font-semibold text-slate-400 hidden md:grid">
            <div className="col-span-1 text-center">HẠNG</div><div className="col-span-3">TỪ VỰNG</div><div className="col-span-3">TIẾN TRÌNH</div><div className="col-span-3 text-center">THƯỞNG CẤP</div><div className="col-span-2 text-center">HÀNH ĐỘNG</div>
          </div>
          <div className="flex flex-col gap-2">
            {sortedVocabulary.length > 0 ? (sortedVocabulary.map((item, index) => (<VocabularyRow key={item.word} item={item} rank={index + 1} onClaim={handleClaim} />))) : (<div className="text-center py-12 text-slate-400"><p>Chưa có dữ liệu thành tựu.</p><p className="text-sm">Hãy hoàn thành các bài học để bắt đầu!</p></div>)}
          </div>
        </main>
...
function VocabularyRow({ item, rank, onClaim }: { item: VocabularyItem, rank: number, onClaim: (word: string) => void }) {
  ...
  return (<div className="grid grid-cols-12 gap-x-4 gap-y-3 items-center p-4 bg-slate-800/70 rounded-xl border border-slate-700/80 hover:bg-slate-700/60 hover:border-cyan-500/50 transition-all duration-300">
      ...
      <div className="col-span-10 md:col-span-3"><p className="font-bold text-lg text-white">{word}</p><span className="md:hidden text-xs text-slate-400">{`Cấp ${level}`}</span></div>
      <div className="col-span-12 md:col-span-3 md:px-2">
        <div className="w-full bg-slate-700 rounded-full h-3"><div className="bg-gradient-to-r from-teal-400 to-cyan-500 h-3 rounded-full transition-all duration-500 ease-out" style={{ width: `${progressPercentage}%` }}></div></div><p className="text-xs text-slate-400 mt-1.5 text-right font-mono">{exp} / {maxExp} EXP</p>
      </div>
      <div className="col-span-6 md:col-span-3 flex items-center justify-center"><div className="flex w-full max-w-[180px] items-center justify-center gap-4 rounded-xl bg-black/20 p-2 shadow-inner border border-slate-700">
        <div className="flex items-center gap-1.5" title="1 Mastery"><MasteryCardIcon className="w-6 h-6 flex-shrink-0" /><span className="text-sm font-semibold text-slate-200">x1</span></div><div className="h-6 w-px bg-slate-600"></div><div className="flex items-center gap-1.5" title={`${goldReward} Vàng`}><GoldIcon className="w-5 h-5 flex-shrink-0" /><span className="text-sm font-semibold text-slate-200">{goldReward}</span></div>
      </div></div>
      <div className="col-span-6 md:col-span-2 flex justify-end md:justify-center"><button onClick={() => onClaim(word)} disabled={!isClaimable} className={`flex items-center justify-center gap-2 w-auto px-3 py-2 rounded-lg font-semibold text-sm transition-all duration-300 border ${isClaimable ? 'bg-gradient-to-r from-emerald-400 to-teal-400 border-emerald-500/50 text-white hover:opacity-90 shadow-lg shadow-emerald-500/20 transform hover:scale-105 cursor-pointer' : 'bg-slate-800 border-slate-700 text-slate-500 cursor-not-allowed'}`}><TrophyIcon className="w-4 h-4" />{isClaimable ? 'Claim' : 'Chưa Đạt'}</button></div>
  </div>);
}
