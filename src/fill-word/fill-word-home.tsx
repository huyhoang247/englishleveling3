...
// --- ĐÂY LÀ CÁCH KHAI BÁO CHÍNH XÁC VÀ DỄ NHÌN NHẤT ---
// Mỗi cụm từ được đặt trên một dòng riêng để dễ đọc,
// được bọc trong dấu ngoặc đơn và kết thúc bằng dấu phẩy.
const PHRASE_LIST = [
    'Primary source',
'Reliable source',
'Energy source',
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (selectedPractice % 100 === 1) {
        ...
        } else if (selectedPractice % 100 === 5) {
             exampleData.forEach(sentence => {
                const wordsInSentence = userVocabularyWords.filter(vocabWord => new RegExp(`\\b${vocabWord}\\b`, 'i').test(sentence.english));
        ...
        } else if (selectedPractice % 100 === 6) {
             exampleData.forEach(sentence => {
                const wordsInSentence = userVocabularyWords.filter(vocabWord => new RegExp(`\\b${vocabWord}\\b`, 'i').test(sentence.english));
            ...
            const generatedQuestions = new Map<string, VocabularyItem>(); 

            userVocabularyWords.forEach(vocabWord => {
                const vocabWordRegex = new RegExp(`\\b${vocabWord}\\b`, 'i');
                const matchingPhrases = PHRASE_LIST.filter(p => vocabWordRegex.test(p));
                ...
                if (matchingPhrases.length > 0) {
                    matchingPhrases.forEach(phrase => {
                        ...
                        matchingSentences.forEach(sentence => {
                            if (!generatedQuestions.has(sentence.english)) {
                                const highlightedPart = phrase.replace(vocabWordRegex, '').trim();
                                const questionPlaceholder = "::P8_PLACEHOLDER::";
                                let questionText = sentence.english.replace(phraseRegex, questionPlaceholder);

                                generatedQuestions.set(sentence.english, {
                                    word: vocabWord,
                                    question: questionText,
                                    vietnameseHint: sentence.vietnamese,
            ...
            gameVocabulary = Array.from(generatedQuestions.values());
        }
                ...
                { (selectedPractice % 100 !== 1) && currentWord && (
                  <div className="bg-white/15 backdrop-blur-sm rounded-lg p-4 shadow-lg border border-white/25 relative overflow-hidden mt-4">
                    <p className="text-lg sm:text-xl font-semibold text-white leading-tight">
                        {currentWord.isPractice8 ? (
                            currentWord.question?.split('::P8_PLACEHOLDER::').map((part, i, arr) => (
                                <React.Fragment key={i}>
                                    {part}
                                    {i < arr.length - 1 && (
                                        <>
                                            <span className="font-bold text-blue-300">{currentWord.highlightedPart}</span>
                                            {' '}
                                            <span className="font-bold text-indigo-300">___</span>
                                        </>
                                    )}
                                </React.Fragment>
                            ))
                        ) : (
                             currentWord.question?.split('___').map((part, i, arr) => ( <React.Fragment key={i}> {part} {i < arr.length - 1 && <span className="font-bold text-indigo-300">___</span>} </React.Fragment> ))
                        )}
                    </p>
