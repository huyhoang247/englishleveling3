...
export default function VocabularyGame({ onGoBack, selectedPractice }: VocabularyGameProps) {
  ...
  const resetP3State = (wordItem: VocabularyItem | null) => {
    if ((selectedPractice === 3 || selectedPractice === 103) && wordItem) {
  ...
  const handleP3WordCheck = useCallback(() => {
  ...
  const p3CurrentWordLength = useMemo(() => {
    if ((selectedPractice !== 3 && selectedPractice !== 103) || !currentWord || activeBlankIndex === null) {
      return Infinity;
    }
    const correctWords = currentWord.word.split(' ');
            ...
            <>
              ...
              {currentWord ? (
                <div className="w-full mt-6 space-y-6">
                  ...
                  {(selectedPractice === 3 || selectedPractice === 103) ? (
                      ...
                      {activeBlankIndex !== null && !isCorrect && (
                        <div className="w-full flex flex-col items-center gap-4 transition-all duration-300">
                          <div className="w-full">
                            <VirtualKeyboard userInput={userInput} setUserInput={setUserInput} wordLength={p3CurrentWordLength} disabled={!!isCorrect}/>
                            <div className="flex justify-center mt-2">
                              <button onClick={handleP3WordCheck} disabled={!userInput.trim()} className="px-8 py-2 rounded-lg font-medium text-sm transition-all duration-200 shadow-sm flex items-center bg-gradient-to-r from-blue-400 to-indigo-500 text-white hover:shadow-md hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0">
