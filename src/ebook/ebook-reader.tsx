...
import { Vocabulary, Playlist } from './GamePage.tsx'; // Lấy type từ GamePage
...
interface BookSidebarProps { isOpen: boolean; onClose: () => void; book: Book; isDarkMode: boolean; toggleDarkMode: () => void; }
        ...
        <div className="p-4 space-y-6 overflow-y-auto flex-grow">
          ...
          <div>
            ...
            <div className="flex items-center justify-between p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
              ...
              <button onClick={toggleDarkMode} className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 focus:ring-blue-500 ${isDarkMode ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`} role="switch" aria-checked={isDarkMode}><span className="sr-only">Chuyển đổi chế độ tối</span><span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform duration-200 ease-in-out ${isDarkMode ? 'translate-x-6' : 'translate-x-1'}`} /></button>
            </div>
          </div>
        </div>
...
const BookStatsModal: React.FC<BookStatsModalProps> = ({ isOpen, onClose, stats, bookTitle, vocabMap }) => {
    ...
    const { inDictionaryWords, outOfDictionaryWords } = useMemo(() => {
        const inDict: [string, number][] = []; const outDict: [string, number][] = [];
...
const EbookReader: React.FC<EbookReaderProps> = ({
    ...
    const bookVocabularyCardIds = useMemo(() => {
        ...
        const allWords = book.content.match(/\b\w+\b/g) || [];
        allWords.forEach(word => {
            const normalizedWord = word.toLowerCase();
            if (vocabMap.has(normalizedWord)) { wordsInBook.add(normalizedWord); }
        });
    ...
    useEffect(() => {
        setIsSidebarOpen(false); // Đảm bảo sidebar đóng khi chuyển sách
        if (audioPlayerRef.current) {
            audioPlayerRef.current.pause();
        }
        setIsAudioPlaying(false);
    }, [book.id]); // Chạy lại khi book thay đổi
    ...
    }, []); // Chạy một lần khi component unmount
    ...
    const renderBookContent = () => {
        ...
        if (!book) return <div className="text-center p-10 text-gray-500 dark:text-gray-400">Không tìm thấy nội dung sách.</div>;
                    ...
                    <div className="mb-6 sm:mb-8 pb-4 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div className="mt-6 flex justify-center">
                            <div className="inline-flex rounded-md shadow-sm bg-gray-100 dark:bg-gray-700 p-1">
                                <button onClick={() => setHighlightMode('word')} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${highlightMode === 'word' ? 'bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 shadow' : 'text-gray-500 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>In đậm từ đơn</button>
                                <button onClick={() => setHighlightMode('phrase')} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${highlightMode === 'phrase' ? 'bg-white dark:bg-gray-900 text-green-600 dark:text-green-400 shadow' : 'text-gray-500 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>In đậm cụm từ</button>
                            </div>
                        </div>
                    </div>
                    {renderBookContent()}
                    ...
                    <div className="max-w-3xl mx-auto flex flex-col items-center gap-2">
                        {availableVoices.length > 1 && (
                            <div className="flex justify-center w-full">
                                <VoiceStepper currentVoice={selectedVoiceKey || '...'} onNavigate={handleVoiceChange} availableVoiceCount={availableVoices.length} />
                            </div>
                        )}
