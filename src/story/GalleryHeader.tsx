// src/components/GalleryHeader.tsx (Đầy đủ)
...
interface GalleryHeaderProps {
  activeTab: 'collection' | 'favorite';
  onTabChange: (tab: 'collection' | 'favorite') => void;
  totalCollection: number;
  favoriteCount: number;
  onToggleSidebar: () => void;
  layoutMode: 'single' | 'double';
  onLayoutModeChange: (mode: 'single' | 'double') => void;
  visualStyle: string;
...
const GalleryHeader = memo(({
  activeTab, onTabChange, totalCollection, favoriteCount, onToggleSidebar,
  layoutMode, onLayoutModeChange, visualStyle, onVisualStyleChange,
  playlists, selectedPlaylistId, onSelectPlaylist, onTogglePin, onDeletePlaylist
}: GalleryHeaderProps) => {
  ...
  const pillsToDisplay = useMemo(() => {
    ...
    const displaySet = new Set<Playlist>(pinned);
  ...
  const handleTogglePinWrapper = async (playlistId: string) => {
    if (!playlists.find(p => p.id === playlistId)?.isPinned && pinnedCount >= 2) {
      alert("Bạn chỉ có thể ghim tối đa 2 playlist.");
      return;
    }
    setIsUpdatingPlaylists(true);
    await onTogglePin(playlistId);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsUpdatingPlaylists(true);
    await onDeletePlaylist(playlistToDelete);
  ...
  return (
    <>
      <div className="w-full max-w-6xl py-6 mx-auto flex-shrink-0">
        <div className="flex justify-between items-center mb-4 px-4">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Flashcard Gallery</h1>
          <div className="flex items-center space-x-2">
            <button onClick={onToggleSidebar} className="relative flex items-center justify-center p-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 transition-all duration-300 cursor-pointer hover:border-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900 ring-2 ring-indigo-100 dark:ring-indigo-800" aria-label="Toggle Sidebar">
        ...
        <div className="inline-flex rounded-lg bg-white dark:bg-gray-800 p-1 mb-4 shadow-sm border border-gray-200 dark:border-gray-700 mx-4">
          <button onClick={() => onTabChange('collection')} className={`flex items-center space-x-1.5 px-4 py-2 text-sm rounded-lg transition-all duration-300 ${activeTab === 'collection' ? 'bg-indigo-50 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 font-medium shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
            ...
            <span className={`inline-flex items-center justify-center ${activeTab === 'collection' ? 'bg-indigo-100 dark:bg-indigo-800 text-indigo-800 dark:text-indigo-200' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'} text-xs font-medium px-1.5 py-0.5 rounded-full ml-1`}>{totalCollection}</span>
          </button>
          <button onClick={() => onTabChange('favorite')} className={`flex items-center space-x-1.5 px-4 py-2 text-sm rounded-lg transition-all duration-300 ${activeTab === 'favorite' ? 'bg-pink-50 dark:bg-pink-900 text-pink-700 dark:text-pink-300 font-medium shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-pink-600 dark:hover:text-pink-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
          ...
          <div className="px-4 mb-6">
            <div className="w-full">
              ...
              <div className="flex items-center space-x-2 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
                <button onClick={() => onSelectPlaylist('all')} className={`flex-shrink-0 flex items-center space-x-2 px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 border ${selectedPlaylistId === 'all' ? 'bg-pink-50 text-pink-700 border-pink-200 dark:bg-pink-900/50 dark:text-pink-300 dark:border-pink-700 font-bold shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:border-pink-300 dark:hover:border-pink-600'}`}>
                  <span>Tất cả</span>
              ...
              <div className="overflow-y-auto px-4 pb-4 flex-grow">
                <ul className="space-y-1">
                  {playlists.filter(p => p.name.toLowerCase().includes(playlistSearch.toLowerCase())).map(p => (
                    <li key={p.id}>
                      <div onClick={() => { onSelectPlaylist(p.id); setShowAllPlaylistsModal(false); setPlaylistSearch(''); }} className={`w-full flex items-center text-left p-3 rounded-lg transition-colors duration-200 group cursor-pointer ${selectedPlaylistId === p.id ? 'bg-pink-100 dark:bg-pink-900/60' : 'hover:bg-gray-100 dark:hover:bg-gray-700/50'}`}>
