...
// --- ICONS (Thêm hiệu ứng đổ bóng để nổi bật trên nền tối) ---
const icons = {
  coin: (
  ...
  heart: (
    <svg xmlns="http://www.w3.org/2000/svg" className="drop-shadow-[0_2px_4px_rgba(255,79,79,0.5)]" width="48" height="48" viewBox="0 0 24 24" fill="#FF4F4F" stroke="#C00" strokeWidth="0.8">
      <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
    </svg>
  ),
  sword: (
    <svg xmlns="http://www.w3.org/2000/svg" className="drop-shadow-[0_2px_4px_rgba(192,192,192,0.5)]" width="48" height="48" viewBox="0 0 24 24" fill="#C0C0C0" stroke="#A9A9A9" strokeWidth="0.8">
  ...
  shield: (
    <svg xmlns="http://www.w3.org/2000/svg" className="drop-shadow-[0_2px_4px_rgba(100,149,237,0.5)]" width="48" height="48" viewBox="0 0 24 24" fill="#6495ED" stroke="#4169E1" strokeWidth="0.8">
...
// --- COMPONENT STATCARD VỚI GIAO DIỆN KIM LOẠI TỐI & VIỀN NEON ---
const StatCard = ({ stat, onUpgrade }) => {
  ...
  return (
    <div className="relative bg-slate-800/60 backdrop-blur-sm rounded-xl w-32 sm:w-36 md:w-40 text-center font-bold text-slate-300 transform transition-all duration-300 hover:scale-105 group">
      {/* Lớp viền Gradient phát sáng */}
      <div className="absolute -inset-0.5 bg-gradient-to-r from-fuchsia-600 to-blue-600 rounded-xl blur-sm opacity-50 group-hover:opacity-100 group-hover:blur-md transition duration-300"></div>
      
      <div className="relative border border-slate-700 rounded-xl">
        {/* Header của thẻ */}
        <div className="bg-black/40 py-1 px-4 rounded-t-xl shadow-inner">
          <p className="text-cyan-300 text-shadow-sm tracking-widest">LV.{level}</p>
        </div>
        ...
        <div className="p-3 flex flex-col items-center gap-2">
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-fuchsia-500 to-purple-600 text-white border border-fuchsia-400/50 rounded-lg py-2 px-3 flex items-center justify-center gap-2 shadow-md hover:shadow-lg hover:shadow-fuchsia-500/40 hover:from-fuchsia-400 hover:to-purple-500 active:shadow-inner active:translate-y-px transition-all duration-200"
          >
...
// --- COMPONENT CHÍNH CỦA ỨNG DỤNG VỚI GIAO DIỆN DARK FANTASY ---
export default function App() {
  ...
  const handleUpgrade = (statId) => {
    ...
    if (gold >= upgradeCost) {
      ...
      setStats(prevStats =>
        prevStats.map(stat =>
          stat.id === statId ? { ...stat, level: stat.level + 1 } : stat
        )
      );
      setMessage('');
    } else {
    ...
    <>
      <style>
        {`
          ...
          .text-shadow-glow { text-shadow: 0 0 8px rgba(255, 255, 255, 0.3), 0 0 12px rgba(0, 150, 255, 0.3); }

          .text-gradient-neon {
            background-image: linear-gradient(45deg, #ec4899, #8b5cf6, #06b6d4);
            -webkit-background-clip: text;
            background-clip: text;
            color: transparent;
          }

          /* Hiệu ứng ánh sáng lướt qua cho thanh tiến trình */
          .progress-bar-sheen::after {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
      ...
      <div className="w-full min-h-screen dark-bg-pattern p-4 flex flex-col items-center justify-center font-lilita text-white overflow-hidden">
        <div className="w-full max-w-md mx-auto">
          
          <header className="w-full flex justify-end mb-4">
              <div className="bg-slate-800/50 backdrop-blur-sm rounded-full py-1.5 px-5 flex items-center gap-3 border border-slate-600 shadow-lg shadow-black/30">
                  <div className="w-6 h-6">{icons.coin}</div>
                  <span className="text-xl text-yellow-300 text-shadow-glow font-bold tracking-wider">{gold.toLocaleString()}</span>
              </div>
          </header>

          <h1 className="text-5xl md:text-6xl text-center text-gradient-neon drop-shadow-[0_2px_10px_rgba(139,92,246,0.4)] my-4">
            Warrior Forge
          </h1>

          <div className="my-6 flex justify-center">
            <div className="relative w-48 h-48 bg-black/20 rounded-full flex items-center justify-center border-2 border-slate-700/70 shadow-inner shadow-black/50">
                <div className="absolute inset-0 rounded-full shadow-[0_0_25px_5px_rgba(6,182,212,0.4)] animate-pulse-slow"></div>
                <svg width="180" height="180" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-[0_5px_10px_rgba(0,0,0,0.6)]">
                    ...
                    <g transform="translate(0, 10)">
                        ...
                        <g transform="rotate(45 150 50)"><rect x="145" y="0" width="10" height="50" fill="#C0C0C0" stroke="#808080" strokeWidth="2"/><rect x="140" y="45" width="20" height="8" fill="#8B4513" stroke="#5C2E00" strokeWidth="2"/><circle cx="150" cy="60" r="5" fill="#FFD700"/></g>
                        <g transform="rotate(-15 40 120)"><ellipse cx="40" cy="120" rx="15" ry="20" fill="#A0522D" stroke="#5C2E00" strokeWidth="2"/><circle cx="40" cy="120" r="5" fill="#FFD700"/></g>
                    </g>
                </svg>
            </div>
          </div>
          ...
          <div className="w-full max-w-sm mx-auto px-2 mt-4 mb-10">
            <div className="text-center mb-2 text-lg text-cyan-300 text-shadow-glow tracking-wider">
              Prestige {prestigeLevel}
            </div>
            {/* Thanh tiến trình năng lượng */}
            <div className="relative w-full h-6 bg-black/50 rounded-full border border-slate-700 shadow-inner shadow-black/50 overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-fuchsia-600 via-purple-500 to-cyan-500 rounded-full transition-all duration-500 ease-out progress-bar-sheen"
