...
/* --- Cài đặt chung & Nền --- */
body { background-color: #0a0a14; background-image: radial-gradient(circle at center, #16213e, #0a0a14); color: #e0e0e0; font-family: 'Roboto', sans-serif; margin: 0; height: 100vh; overflow: hidden; }
#root { width: 100%; height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 20px; box-sizing: border-box; }
.main-header { position: fixed; top: 0; left: 0; width: 100%; padding: 12px 25px; display: flex; justify-content: space-between; align-items: center; background-color: rgba(16, 22, 46, 0.7); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); border-bottom: 1px solid rgba(255, 255, 255, 0.1); z-index: 100; box-sizing: border-box; transition: opacity 0.3s ease; }
.header-title { font-size: 1.25rem; font-weight: 600; color: #e0e0e0; margin: 0; text-shadow: 0 1px 3px rgba(0,0,0,0.5); }
.vocab-screen-close-btn { width: 44px; height: 44px; background: transparent; border: none; cursor: pointer; display: flex; justify-content: center; align-items: center; transition: transform 0.2s ease, opacity 0.2s ease; opacity: 0.9; margin: -10px; padding: 10px; }
.vocab-screen-close-btn:hover { transform: scale(1.15); opacity: 1; }
.vocab-screen-close-btn img { width: 24px; height: 24px; filter: drop-shadow(0 1px 2px rgba(0,0,0,0.5)); }
.chest-gallery-container { display: flex; flex-wrap: wrap; justify-content: center; gap: 30px; width: 100%; max-width: 1300px; overflow-y: auto; padding: 20px; max-height: calc(100vh - 101px); box-sizing: border-box; margin-top: 61px; }
...
.chest-ui-container { width: 100%; max-width: 380px; min-width: 300px; background-color: #1a1f36; border-radius: 16px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.5), 0 0 15px rgba(76, 89, 186, 0.2); overflow: hidden; display: flex; flex-direction: column; transition: transform 0.3s ease, box-shadow 0.3s ease; position: relative; border: none; }
.chest-ui-container::before { content: ''; position: absolute; inset: 0; border-radius: 16px; padding: 1px; background: linear-gradient(135deg, rgba(129, 140, 248, 0.4), rgba(49, 46, 129, 0.3)); -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0); -webkit-mask-composite: xor; mask-composite: exclude; pointer-events: none; }
.chest-ui-container:hover { transform: translateY(-8px); box-shadow: 0 15px 35px rgba(0, 0, 0, 0.6), 0 0 25px rgba(129, 140, 248, 0.3); }
.chest-header { padding: 12px 20px; background-color: rgba(42, 49, 78, 0.7); font-size: 0.9rem; font-weight: 600; color: #c7d2fe; text-align: center; text-transform: uppercase; letter-spacing: 0.5px; }
.chest-body { background: linear-gradient(170deg, #43335b, #2c2240); padding: 25px 20px 20px 20px; position: relative; flex-grow: 1; display: flex; flex-direction: column; align-items: center; overflow: hidden; }
.chest-body::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-image: url('data:image/svg+xml,...'); opacity: 0.1; z-index: 0; }
.chest-body > * { position: relative; z-index: 1; }
.chest-title { font-size: clamp(1.4rem, 4vw, 1.75rem); color: white; font-weight: 900; text-shadow: 1px 1px 4px rgba(0,0,0,0.5); margin: 0 0 20px; text-align: center; }
.help-icon { position: absolute; top: 15px; right: 15px; width: 24px; height: 24px; border-radius: 50%; background-color: rgba(0, 0, 0, 0.3); border: 1px solid rgba(255,255,255,0.5); color: white; font-size: 16px; font-weight: bold; cursor: pointer; display: flex; justify-content: center; align-items: center; transition: all 0.2s ease; z-index: 2; }
...
.info-bubble { flex: 2; background-color: rgba(10, 10, 20, 0.6); color: #d1d5db; padding: 10px 15px; border-radius: 8px; border: 1px solid rgba(255, 255, 255, 0.1); font-size: 0.85rem; text-align: left; }
...
.chest-button { flex: 1; padding: 12px; border-radius: 10px; border: none; cursor: pointer; transition: transform 0.1s ease, box-shadow 0.1s ease; color: #ffffff; font-weight: 700; font-size: 0.95rem; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.4); box-shadow: inset 0 -3px 0 rgba(0,0,0,0.25); display: flex; align-items: center; justify-content: center; gap: 8px; }
...
.button-price { display: flex; align-items: center; justify-content: center; gap: 6px; font-size: 0.85rem; color: white; font-weight: 600; background-color: rgba(0,0,0,0.2); padding: 3px 8px; border-radius: 12px; text-shadow: none; }
...
.card-wrapper.dealt-in { animation: deal-in 0.5s ease-out forwards; opacity: 0; }
`}</style> );

// ========================================================================
// === 2. DỮ LIỆU CỐ ĐỊNH (Không đổi) =======================================
// ========================================================================
const CHEST_DATA = [
...
// ========================================================================
// === 3. CÁC COMPONENT CON (Đã được sửa đổi) ===========================
// ========================================================================

const Card = ({ cardData, isFlipped }: { cardData: { image: string }, isFlipped: boolean }) => (
    <div className={`card-container ${isFlipped ? 'flipped' : ''}`}>
        <div className="card-inner">
            <div className="card-face card-back">?</div>
            <div className="card-face card-front">
                <img src={cardData.image} alt="Revealed Card" className="card-image-in-card" />
            </div>
        </div>
    </div>
);

const SingleCardOpener = ({ card, onClose, onOpenAgain }: { card: ImageReward, onClose: () => void, onOpenAgain: () => void }) => {
    const [isFlipped, setIsFlipped] = useState(false);
    useEffect(() => {
        const t = setTimeout(() => setIsFlipped(true), 500);
        return () => clearTimeout(t);
    }, [card]); // Re-trigger animation if the card changes

    return (<>
        <div style={{ textAlign: 'center' }}>
            <div style={{ display: 'inline-block', maxWidth: '250px', width: '60vw', marginBottom: '20px' }}>
                <Card cardData={{ image: card.url }} isFlipped={isFlipped} />
            </div>
        </div>
        <div className="overlay-footer">
            <button onClick={onOpenAgain} className="footer-btn primary">Mở Lại</button>
            <button onClick={onClose} className="footer-btn">Đóng</button>
        </div>
    </>);
};

const FourCardsOpener = ({ cards, onClose, onOpenAgain }: { cards: ImageReward[], onClose: () => void, onOpenAgain: () => void }) => {
    const [flippedIndices, setFlippedIndices] = useState<Set<number>>(new Set());
    
    useEffect(() => {
        setFlippedIndices(new Set()); // Reset on new set of cards
        ...
        }, 800)); // Start flipping after cards are dealt in
        ...
        <div style={{ textAlign: 'center' }}>
            <div className="four-card-grid-container">
                {cards.map((card, index) => (
                    <div key={card.id} className="card-wrapper dealt-in" style={{ animationDelay: `${index * 100}ms` }}>
                        <Card cardData={{ image: card.url }} isFlipped={flippedIndices.has(index)} />
                    </div>
                ))}
            </div>
        </div>
        <div className="overlay-footer">
            <button onClick={onOpenAgain} className="footer-btn primary">Mở Lại x4</button>
            <button onClick={onClose} className="footer-btn">Đóng</button>
        </div>
    </>);
};

interface ChestUIProps { /* ... props như cũ ... */ }
const ChestUI: React.FC<any> = ({ headerTitle, mainTitle, imageUrl, infoText, pityLine1, pityLine2, price1, price10, onOpen1, onOpen10 }) => {
            ...
            <main className="chest-body">
                ...
                <div className="action-button-group" style={{ marginTop: 'auto', paddingTop: '20px' }}>
                    <button className={`chest-button ${isFree ? 'btn-free' : 'btn-get-1'}`} onClick={onOpen1}>
                        ...
                        {typeof price1 === 'number' && (<span className="button-price"><img src="https://raw.githubusercontent.com/huyhoang247/englishleveling3/refs/heads/main/src/icon/dollar.png" alt="price icon" className="price-icon" />{price1}</span>)}
...
// ========================================================================
// === 4. COMPONENT CHÍNH (APP) (Sửa đổi chính) ===========================
// ========================================================================
interface VocabularyChestScreenProps {
    onClose: () => void;
    currentUserId: string | null; // Cần truyền userId vào
}

const VocabularyChestScreen: React.FC<VocabularyChestScreenProps> = ({ onClose, currentUserId }) => {
    const [overlayMode, setOverlayMode] = useState<'single' | 'four' | null>(null);
    const [revealedCards, setRevealedCards] = useState<ImageReward[]>([]);
    
    // --- SỬ DỤNG HOOK ĐỂ LẤY DỮ LIỆU ---
    const { isLoading, availableCount, getNextRewards } = useUniqueImageReward(currentUserId);

    // Hàm xử lý việc mở thẻ
    const handleOpen = (count: 1 | 4) => {
        if (!currentUserId) {
            alert("Vui lòng đăng nhập để sử dụng tính năng này.");
            return;
        }
        if (availableCount < count) {
            alert(`Không đủ hình ảnh để mở! (Cần ${count}, chỉ còn ${availableCount})`);
            return;
        }

        const newRewards = getNextRewards(count);
        if (newRewards.length > 0) {
            setRevealedCards(newRewards);
            setOverlayMode(count === 1 ? 'single' : 'four');
        } else {
             alert('Đã có lỗi xảy ra hoặc đã hết hình ảnh. Vui lòng thử lại!');
        }
    };
    ...
    const handleCloseOverlay = () => {
        setOverlayMode(null);
        setRevealedCards([]); // Xóa thẻ cũ
    };
    
    const showHeader = overlayMode === null;
            ...
            {showHeader && (
                <header className="main-header">
                    <h1 className="header-title">
                        Chọn Rương ({isLoading ? "Đang tải..." : `${availableCount} ảnh còn lại`})
                    </h1>
            ...
            <div className="chest-gallery-container">
                {CHEST_DATA.map((chest) => (
                    <ChestUI
                        key={chest.id}
                        {...chest}
                        onOpen1={() => handleOpen(1)}
                        onOpen10={chest.price10 !== null ? () => handleOpen(4) : undefined}
                    />
                ))}
            </div>

            {overlayMode === 'single' && revealedCards.length > 0 && (
                <div className="card-opening-overlay">
                    <div className="overlay-content">
                        <SingleCardOpener 
                            key={revealedCards[0].id} 
                            card={revealedCards[0]} 
                            onClose={handleCloseOverlay}
                            onOpenAgain={() => handleOpen(1)}
                        />
                    </div>
                </div>
            )}
            {overlayMode === 'four' && revealedCards.length > 0 && (
                <div className="card-opening-overlay">
                    <div className="overlay-content">
                        <FourCardsOpener 
                            key={revealedCards.map(c => c.id).join('-')}
                            cards={revealedCards} 
                            onClose={handleCloseOverlay} 
                            onOpenAgain={() => handleOpen(4)}
                        />
                    </div>
                </div>
            )}
