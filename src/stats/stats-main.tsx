...
<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon>,
    ...
    Trophy: <g><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6M18 9h1.5a2.5 2.5 0 0 0 0-5H18M4 22h16M10 14l2 2 2-2M12 17v5"></path><path d="M12 17a5 5 0 0 1-5-5V5a2 2 0 0
1 2-2h6a2 2 0 0 1 2 2v7a5 5 0 0 1-5 5z"></path></g>,
    ...
    Gem: <g><path d="M6 3h12l4 6-10 13L2 9l4-6z"></path><path d="M12 22L4 9l8-6 8
6-8 13z"></path><path d="M12 2l8 7-8 7-8-7 8-7z"></path><path d="M2 9h20"></path><path d="M12 2v20"></path></g>,
...
interface CharacterCardProps {
  onClose?: () => void;
// Optional function to call when closing
}
...
export default function CharacterCard({ onClose }: CharacterCardProps) {
 ...
 // HP increases by 25 per point
      } else {
        newTempStats[stat] += 2;
 // Other stats increase by 2 per point
      }
 ...
 // Get the original stat value before allocation
  ...
  const cancelChanges = () => {
    ...
    const pointsAdded = Object.entries(tempStats).reduce((total, [key, value]) => {
        ...
        if (key === 'hp') {
          ...
          return total +
 Math.floor((value - originalValue) / 25);
        } else {
 ...
 w-6 h-6 ${colorScheme.bg} opacity-20 rounded-full`}></div>
 ...
 (
        ...
        >
                    ...
                    <div className="flex items-center justify-center">
                      ...
                      <p className={`text-sm font-bold ${isIncreased ? 'text-green-600' : colorScheme.text} bg-gray-50 rounded-md px-2 py-0.5 border ${isIncreased ?
 'border-green-100' : isSpecial ? 'border-red-100' : 'border-gray-200'}`}>
                        {isSpecial ?
 stats[stat.key].toLocaleString() : stats[stat.key]} {/* Format HP with commas */}
                      </p>
                    </div>
                ...
                onClick={() => increaseStat(stat.key)}
                      ...
                      className={`flex items-center justify-center w-6 h-6 rounded-md ${statPoints <= 0 ?
 'text-gray-300 cursor-not-allowed' : 'hover:bg-gray-100 text-gray-500'}`}
                    >
 ...
 inset-0 rounded-full bg-gradient-to-br from-amber-500 to-yellow-400 animate-pulse"></div>
              ...
              <div className="flex">
                ...
                <p className="text-red-700 text-xs">
                    {exchangeDirection === 'coinToPoint' ?
                      ...
                      `Bạn cần tối thiểu
 ${exchangeAmount.toLocaleString()} Point để thực hiện chuyển đổi này.`
                    }
                  </p>
                </div>
              </div>
        ...
        <div className="h-20 relative bg-white flex-shrink-0"> {/* Added flex-shrink-0 to prevent header shrinking */}
          ...
          <div className="absolute top-2 right-4 flex items-center space-x-2 overflow-hidden
            ...
            <div className="overflow-hidden">
              <div className={`flex items-center p-1 pl-2 pr-1.5 rounded-lg bg-gradient-to-r from-amber-500 to-yellow-500 shadow-md ${coinBadgePulse ?
 'animate-pulse' : ''}`}>
     ...
     <span className="text-xs font-bold text-white">{character.coins.toLocaleString()}</span>
                 ...
                 <button
                    ...
                    title="Chuyển đổi
 Coin sang Point" // Tooltip remains
                  >
          ...
          Exchange
            ...
            {/* Points Badge */}
            <div className="overflow-hidden">
              <div className={`flex items-center p-1 pl-2 pr-1.5 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 shadow-md ${pointBadgePulse ?
 'animate-pulse' : ''}`}>
                <div className="w-4 h-4 mr-1.5 relative">
     ...
     <span className="text-xs font-bold text-white">{statPoints}</span>
                ...
                <button
                  ...
                  className={`ml-1.5 w-4 h-4 rounded-full bg-white bg-opacity-20 hover:bg-opacity-30 flex items-center justify-center
 transition-colors ${!showPointsPanel && statPoints > 0 ? '' : 'invisible pointer-events-none'}`} // Visible only if panel is closed and points > 0
 ...
 (
      ...
      <button
                ...
                // Render stats in view
 mode (when allocation panel is closed)
 ...
 */}
        ...
        {/* MODIFIED: Ensure footer has space-between for left/right alignment */}
        <div
 className="px-8 py-3 bg-gradient-to-br from-gray-50 to-gray-100 border-t border-gray-200 flex-shrink-0 flex justify-between items-center"> {/* Added flex-shrink-0 and justify-between */}

          {/* Close Button - MOVED TO FOOTER */}
          {onClose && (
            <button
              onClick={onClose}
              className="w-8 h-8 bg-gray-800 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-red-600 transition-colors z-10"
              aria-label="Đóng cửa sổ chỉ số"
            >
              {/* Replaced Icon "X" with img tag */}
              <img
          ...
          {/* NEW: Render the new ResetStatsControl component - Already here */}
          <ResetStatsControl
