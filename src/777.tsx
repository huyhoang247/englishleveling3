...
const GameScreen = ({ room, balance, jackpot, onExit, onGameEnd, onJackpotUpdate }: {
    room: Room;
    balance: number;
    jackpot: number;
    onExit: () => void;
    ...
    }, [reelsResult, bet, jackpot, room, onGameEnd, onJackpotUpdate]);
    
    const handleBetChange = (amount: number) => {
        setBet(prev => {
            const newBet = prev + amount;
            if (newBet >= room.baseBet && newBet <= localBalance) return newBet;
            if (newBet > localBalance) return localBalance;
            return prev;
        });
    };
    useEffect(() => {
        if (bet > localBalance) setBet(localBalance > room.baseBet ? localBalance : room.baseBet);
        if (localBalance < room.baseBet && localBalance > 0) setBet(localBalance);
