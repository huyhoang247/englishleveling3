...
// --- Skeleton cho một dòng từ vựng (Không thay đổi) ---
const SkeletonRow = () => (
...
// Component con cho Skeleton thẻ Stats, sử dụng kỹ thuật "Invisible Placeholder"
const StatCardSkeleton = ({ valuePlaceholder, labelPlaceholder }: { valuePlaceholder: string; labelPlaceholder: string; }) => (
    <div className="flex flex-1 sm:flex-none sm:w-52 items-center gap-3 p-3 bg-slate-800/50 border border-slate-700 rounded-lg">
      {/* Skeleton Icon */}
      <div className="w-7 h-7 bg-slate-700 rounded-md flex-shrink-0 animate-pulse"></div>
      <div className="w-full">
        {/* Dòng trên: Dùng class font thật, nhưng chữ trong suốt và có nền */}
        <p className="text-xl font-bold text-transparent bg-slate-700 rounded-md animate-pulse w-1/2 mb-1.5">
          {valuePlaceholder}
        </p>
        {/* Dòng dưới: Tương tự */}
        <p className="text-sm text-slate-400 text-transparent bg-slate-700 rounded-md animate-pulse w-full">
          {labelPlaceholder}
        </p>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-800 to-slate-900 text-white font-sans flex flex-col items-center">
      ...
      <div className="w-full max-w-4xl mx-auto p-4 sm:p-8 pt-6">
        ...
        {/* Sử dụng component StatCardSkeleton mới */}
        <section className="mb-6 flex flex-row justify-center items-center gap-4">
          <StatCardSkeleton valuePlaceholder="000" labelPlaceholder="Vocabulary" />
          <StatCardSkeleton valuePlaceholder="000" labelPlaceholder="Mastery" />
        </section>
