...
const StickmanShadowFinal = ({ onClose }) => {
    ...
    const riseImageRef = useRef(null); // Ref cho ảnh Arise
    ...
    const extractShadow = () => {
        ...
        if (closestIndex !== -1) {
            ...
            // Thay đổi ở đây: Type 'ARISE' để vẽ ảnh thay vì text
            addFloatingText(s.x, s.y - 120, "", '#9333ea', 32, 'ARISE');
    ...
    const draw = () => {
        ...
        if (gameState !== 'INIT') {
            ...
            floatingTexts.current.forEach(t => {
                ...
                else if (t.type === 'ARISE' && riseImageRef.current?.complete) {
                    // Logic vẽ ảnh thay vì chữ ARISE!
                    const sc = t.life > 50 ? 1 + (t.life - 50) * 0.05 : 1;
                    ctx.translate(t.x, t.y);
                    ctx.scale(sc, sc);
                    const size = 100;
                    ctx.drawImage(riseImageRef.current, -size / 2, -size / 2, size, size);
                }
                else {
