...
const StickmanShadowFinal = ({ onClose }) => {
    ...
    // --- SỬA ĐỔI HÀM: addFloatingText (Phân biệt Damage vs Heal) ---
    // Damage: Tản ra xung quanh, bay nhanh
    // Heal: Tập trung ở giữa, bay chậm, vị trí cao hơn
    const addFloatingText = (x, y, text, color, size = 20, type = 'TEXT') => {
        ...
        if (isHeal) {
            // HỒI MÁU:
            // 1. Luôn xuất hiện chính giữa (ít random X)
            finalX = x + (Math.random() * 10 - 5);
            // 2. Xuất hiện cao hơn sát thương một chút để không bị che
            finalY = y - 40;
            // 3. Bay lên chậm rãi hơn (cảm giác "thấm" máu)
            velocityY = -1.2;
        } else if (isDamage || isCrit) {
            // SÁT THƯƠNG:
            // 1. Tản ra rộng hơn (trái/phải) để thấy rõ đòn đánh
            finalX = x + (Math.random() * 50 - 25);
            // 2. Tốc độ bay biến thiên (nhanh/chậm) tạo độ sâu
