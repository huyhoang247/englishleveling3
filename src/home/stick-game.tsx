...
// --- COMPONENT HEADER UI ---
// UPDATED: Nhận thêm prop 'coins' thật từ Context
    ...
    return (
        <header className="absolute top-0 left-0 w-full h-[53px] box-border flex items-center justify-between px-4 bg-slate-900/90 border-b border-white/10 z-[60]">
            ...
            <div className="flex items-center gap-3">
                ...
                {/* Coins Display - UPDATED: Sử dụng coin thật từ Context */}
                <CoinDisplay
...
const StickmanShadowFinal = ({ onClose }) => {
    ...
    // UPDATED: Lấy thêm coins và updateCoins từ useGame
    const { totalPlayerStats, getEquippedSkillsDetails, coins, updateCoins } = useGame();
    ...
    const p1 = useRef({
        ...
        damage: 12,
        defense: 0,
        coins: 0, // Vẫn giữ để tracking session nếu cần, nhưng ko dùng hiển thị chính
        state: 'IDLE', dir: 1,
    ...
    // UPDATED: updateLoot nhận thêm viewWidth để tính vị trí bay về Header
    const updateLoot = (floorY, camX, viewWidth) => {
        for (let i = lootCoins.current.length - 1; i >= 0; i--) {
            ...
            if (c.state === 'DROP') {
                ...
                // UPDATE TARGET: Bay về phía Header Top Right (nơi hiển thị coin mới)
                const destX = camX + viewWidth - 120; // Cách mép phải màn hình 1 chút
                const destY = 28; // Giữa header (chiều cao 53)
                
                c.x += (destX - c.x) * 0.08; 
                c.y += (destY - c.y) * 0.08;
                
                if (Math.abs(destX - c.x) < 20 && Math.abs(destY - c.y) < 20) {
                    // CẬP NHẬT TRỰC TIẾP VÀO CONTEXT (Optimistic UI)
                    updateCoins(c.val);
                    // Tracking session (optional)
    ...
    // --- DRAWING FUNCTIONS ---
    const drawHUD = (ctx) => {
        // UPDATED: Đã loại bỏ phần hiển thị Coins tại đây (hudX, hudY)
    ...
    const update = () => {
        ...
        // UPDATED: Pass viewWidth into updateLoot
    ...
    return (
      ...
      `}</style>
            ...
            {/* --- HEADER (Pass Global Coins) --- */}
            <GameHeader onHome={onClose} shadowCount={shadowCount} maxShadows={CFG.maxShadows} coins={coins} />
