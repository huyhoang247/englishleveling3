...
// --- COMPONENT HEADER UI ---
// UPDATED: Nhận prop coins (visual) để hiển thị mượt mà
    ...
    return (
        <header className="absolute top-0 left-0 w-full h-[53px] box-border flex items-center justify-between px-4 bg-slate-900/90 border-b border-white/10 z-[60]">
            ...
            <div className="flex items-center gap-3">
                ...
                {/* Coins Display - Hiển thị số tiền visual */}
                <CoinDisplay
...
const StickmanShadowFinal = ({ onClose }) => {
    ...
    // Lấy coins gốc (từ Server) và hàm updateCoins
    const { totalPlayerStats, getEquippedSkillsDetails, coins: initialServerCoins, updateCoins } = useGame();

    // --- LOGIC TIỀN TỆ (VISUAL & BUFFER) ---
    // 1. State hiển thị (cộng ngay khi nhặt để UI mượt)
    const [visualCoins, setVisualCoins] = useState(initialServerCoins);
    
    // 2. Ref tích lũy tiền trong phiên chơi (Chỉ cộng dồn, không gửi server liên tục)
    ...
    const p1 = useRef({
        ...
        damage: 12,
        defense: 0,
        coins: 0, // Coins session tracking nội bộ (không dùng để save)
        state: 'IDLE', dir: 1,
    ...
    // --- CHỨC NĂNG THOÁT GAME AN TOÀN ---
    // Chỉ gọi hàm này khi người chơi bấm nút Thoát hoặc nút Home
    const handleSafeExit = () => {
        // Nếu có tiền tích lũy, gửi về server 1 lần duy nhất
        if (sessionEarningsRef.current > 0) {
            console.log("Saving session earnings:", sessionEarningsRef.current);
            updateCoins(sessionEarningsRef.current);
        }
        // Gọi callback đóng game (trở về menu chính)
        onClose();
    };
    ...
    // --- CẬP NHẬT COIN: HIỂN THỊ TRƯỚC, LƯU SAU ---
        ...
        for (let i = lootCoins.current.length - 1; i >= 0; i--) {
            ...
            if (c.state === 'DROP') {
                ...
                if (Math.abs(destX - c.x) < 20 && Math.abs(destY - c.y) < 20) {
                    ...
                    p1.current.coins += c.val; // Tracking nội bộ
                    lootCoins.current.splice(i, 1);
                }
            }
        }

        // Nếu có coin nhặt được trong frame này
        if (coinsInFrame > 0) {
            // 1. Cập nhật giao diện (Visual Only) - Rất nhanh
            setVisualCoins(prev => prev + coinsInFrame);
            
            // 2. Cộng dồn vào két sắt tạm (Chưa gửi Server)
    ...
    return (
      ...
      `}</style>
            ...
            {/* --- HEADER (Pass visualCoins & handleSafeExit) --- */}
            ...
            {gameState === 'GAMEOVER' && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 z-[100]">
                    ...
                    <div className='flex gap-4 mt-4'>
                        {/* Hồi sinh: KHÔNG gọi handleSafeExit để giữ nguyên số tiền đã tích lũy trong Ref */}
                        ...
                        {/* Thoát: Gọi handleSafeExit để lưu tiền và thoát */}
