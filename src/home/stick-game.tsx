...
const StickmanShadowFinal = () => {
  ...
  // Thêm tham số `type` để phân biệt EXP
  ...
  const distributeExp = (killer, deadEnemy) => {
      ...
      if (killer.type === 'PLAYER') {
          ...
          // Thay đổi text EXP thành số và gán type = 'EXP'
          addFloatingText(p1.current.x, p1.current.y - 80, `+${totalExp}`, '#fff', 16, 'EXP');
          checkLevelUp(p1.current, true);
      } else if (killer.type === 'ALLY') {
          killer.currentExp += totalExp;
          addFloatingText(killer.x, killer.y - 80, `+${totalExp}`, '#fff', 14, 'EXP');
          checkLevelUp(killer, false);
          
          const shareExp = Math.floor(totalExp * 0.5);
          p1.current.currentExp += shareExp;
          addFloatingText(p1.current.x, p1.current.y - 80, `+${shareExp}`, '#fff', 16, 'EXP');
          checkLevelUp(p1.current, true);
      }
  };
        ...
        floatingTexts.current.forEach(t => {
            ...
            if (t.type === 'EXP') {
                 ...
                 if (expImageRef.current && expImageRef.current.complete) {
                    const iconSize = 22; 
                    // Vẽ Icon
                    ctx.drawImage(expImageRef.current, t.x - 25, t.y - 15, iconSize, iconSize);
                    
                    // Style cho số điểm EXP
