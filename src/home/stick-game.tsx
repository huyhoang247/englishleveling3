...
const StickmanShadowFinal = ({ onClose }) => {
    ...
    const CFG = {
        ...
        riseRange: 150,     // Khoảng cách tối đa để có thể Rise
    };
    ...
    const riseSkillImageRef = useRef(null); // Ref cho icon ARISE
    ...
    useEffect(() => {
        ...
        // Load ảnh cho hiệu ứng "ARISE"
        const rImg = new Image();
        rImg.src = RISE_SKILL_IMG_URL;
        riseSkillImageRef.current = rImg;
    }, []);
    ...
    const extractShadow = () => {
        ...
        // Sử dụng Range từ Config
        let minDst = CFG.riseRange;

        souls.current.forEach((s, i) => {
            const dst = Math.abs(p.x - s.x);
            if (dst <= minDst) { // Check range
                minDst = dst; // Find closest
                closestIndex = i;
            }
        });

        // Auto mode logic (Có thể bỏ qua range nếu muốn, nhưng ở đây ta giữ logic range cho nhất quán)
        if (closestIndex === -1 && isAutoRef.current && souls.current.length > 0) {
             // Nếu auto, ưu tiên con gần nhất bất kể range, hoặc lấy con đầu tiên
             // Để nhất quán với nút bấm, ta chỉ auto lấy nếu trong range.
             // Tuy nhiên, logic cũ là auto lấy con đầu tiên nếu không có con nào gần.
             // Ta giữ logic cũ cho Auto để nó mạnh hơn:
             closestIndex = 0;
        }
        ...
        if (closestIndex !== -1) {
            ...
            // Thay đổi text "ARISE!" thành icon
            addFloatingText(s.x, s.y - 120, "", '#9333ea', 40, 'ARISE_ICON');
    ...
    const addFloatingText = (x, y, text, color, size = 20, type = 'TEXT') => {
        ...
        } else if (type === 'ARISE_ICON') {
            finalY = y - 20; // Xuất hiện thấp hơn một chút để bay lên
            velocityY = -1.5;
        } else {
    ...
    const update = () => {
        ...
        let hasSoulInRange = false; // Flag: Có Soul trong phạm vi hút không?
        ...
        // Chỉ hiện nút Rise sáng lên nếu có Soul TRONG PHẠM VI và chưa full slot
    ...
    const draw = () => {
        ...
        if (gameState !== 'INIT') {
            ...
            floatingTexts.current.forEach(t => {
                ...
                // --- VẼ ICON ARISE THAY VÌ TEXT ---
                else if (t.type === 'ARISE_ICON' && riseSkillImageRef.current?.complete) {
                    ctx.translate(t.x, t.y);
                    // Hiệu ứng zoom nhẹ khi xuất hiện
                    const sc = t.life > 40 ? 1 + (t.life - 40) * 0.05 : 1; 
                    ctx.scale(sc, sc);
                    // Vẽ icon kích thước 50x50 (điều chỉnh kích thước tại đây)
                    const size = 50;
                    ctx.drawImage(riseSkillImageRef.current, -size/2, -size/2, size, size);
                }
                else {
                        ...
                        {!isAuto && !floorTransitioning.current && (
                            <div className="grid grid-cols-2 gap-4 items-end">
                                ...
                                <div className="flex justify-end items-end">
                                    <button
                                        // Vô hiệu hóa khi không có Soul TRONG PHẠM VI hoặc đã full slot
                                        disabled={!canRise}
                                        className={`
                                                group relative
                                                w-16 h-16 
                                                ...
                                                ${!canRise
                                                ? 'opacity-50 grayscale cursor-not-allowed bg-slate-900/40 border-[2px] border-gray-700' // Trạng thái Disable (Xa hoặc ko có Soul)
                                                : isRisePressed
                                                    ? 'opacity-100 scale-90 bg-slate-800 border-[2px] border-purple-400 shadow-[0_0_20px_rgba(168,85,247,0.8)]' // Trạng thái Pressed
                                                    : 'opacity-100 scale-100 bg-slate-900/60 border-[2px] border-gray-500/30 shadow-[0_4px_10px_rgba(0,0,0,0.5)]' // Trạng thái Active (Có Soul gần)
                                            }
                                            `}
