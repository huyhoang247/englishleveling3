...
// Player Sprite Sheet Config
const PLAYER_SPRITES = {
    // Normal/Idle state (Based on prompt data)
    IDLE: {
        ...
        frameH: 259,
        totalFrames: 36,
        duration: 2.0 // Seconds for full loop
    },
    // Run state
    RUN: {
    ...
    ATTACK: {
        ...
        frameH: 279,
        totalFrames: 36,
        duration: 0.6 // Speed up slightly for game responsiveness (Prompt said 3s but that is too slow for combat)
    }
};
...
const StickmanShadowFinal = ({ onClose }) => {
    ...
    useEffect(() => {
        ...
        // Load Player Sprites
        const pIdle = new Image(); pIdle.src = PLAYER_SPRITES.IDLE.url;
        const pRun = new Image(); pRun.src = PLAYER_SPRITES.RUN.url;
        const pAtk = new Image(); pAtk.src = PLAYER_SPRITES.ATTACK.url;
    ...
    // --- PROCEDURAL STICKMAN DRAWING (FOR ENEMIES & SHADOWS) ---
    const drawProceduralStickman = (ctx, p) => {
    ...
    const drawStickman = (ctx, p) => {
        ...
        // --- NEW SPRITE SYSTEM FOR PLAYER ---
        if (p.type === 'PLAYER') {
            let sKey = 'IDLE';
            if (p.state === 'RUN') sKey = 'RUN';
            if (p.state === 'ATTACK') sKey = 'ATTACK';
            if (p.state === 'JUMP') sKey = 'RUN'; // Use Run sprite for Jump

            const config = PLAYER_SPRITES[sKey];
            const img = playerSpriteRefs.current[sKey];

            if (img && img.complete) {
                // Calculate Frame
                // Game logic runs at 60fps. Duration is in seconds.
                // Frames to show = totalFrames.
                // Speed factor = totalFrames / (duration * 60)
                const fps = config.totalFrames / config.duration;
                // Update frame based on global animTimer (which increments +1 per tick)
                // We divide animTimer by 60 to get seconds, multiply by fps
                const currentFrame = Math.floor((p.animTimer / 60) * fps) % config.totalFrames;

                // Scaling Logic
                // We want the character to be roughly 130px tall in game to match old stickman box
                // Original sprites are ~260px tall.
                const targetHeight = 130;
                const scaleRatio = targetHeight / config.frameH;
                const drawWidth = config.frameW * scaleRatio;
                const drawHeight = targetHeight;

                ctx.save();
                // Translate to feet position (x, y)
                ctx.translate(p.x, p.y);
                ctx.scale(p.dir, 1);

                // Draw Image Centered Horizontally, Aligned to Bottom Vertically
                ctx.drawImage(
                    img,
                    currentFrame * config.frameW, 0, config.frameW, config.frameH, // Source
                    -drawWidth / 2, -drawHeight + 3, drawWidth, drawHeight // Destination (+3 offset for ground)
                );

                ctx.restore();
            } else {
                // Fallback while loading
                drawProceduralStickman(ctx, p);
            }
        } 
        // --- OLD SYSTEM FOR ENEMIES & SHADOWS ---
        else {
            drawProceduralStickman(ctx, p);
        }
    ...
    const update = () => {
        ...
        // IMPORTANT: Increment global animation timer for sprites
        usr.animTimer++; 
        ...
        for (let i = souls.current.length - 1; i >= 0; i--) {
            ...
            s.life--; s.anim++;
            if (s.life <= 0) {
                souls.current.splice(i, 1);
            } else {
                if (Math.abs(usr.x - s.x) <= CFG.riseRange) {
                    hasSoulInRange = true;
                }
            }
        }
                            ...
                            <div className="grid grid-cols-2 gap-4 items-end">
                                <div className="flex justify-end items-end">
                                    <button
                                        disabled={!canRise}
                                        className={`
                                                group relative w-16 h-16 rounded-full flex items-center justify-center touch-none select-none transition-all duration-150 ease-out
                                                ${!canRise
                                                ? 'opacity-50 grayscale cursor-not-allowed bg-slate-900/40 border-[2px] border-gray-700'
                                                : isRisePressed
                                                    ? 'opacity-100 scale-90 bg-slate-800 border-[2px] border-purple-400 shadow-[0_0_20px_rgba(168,85,247,0.8)]'
                                                    : 'opacity-100 scale-100 bg-slate-900/60 border-[2px] border-gray-500/30 shadow-[0_4px_10px_rgba(0,0,0,0.5)]'
                                            }
                                            `}
                                    ...
                                    >
                                        ...
                                        <img
                                            ...
                                            alt="Rise"
                                            className={`w-9 h-9 object-contain transition-all duration-150 pointer-events-none ${canRise && isRisePressed ? 'scale-110 brightness-150' : ''}`}
                                        />
                                    </button>
                                </div>
                                <button
                                    className={`
                                        group relative w-20 h-20 rounded-full flex items-center justify-center touch-none select-none transition-all duration-100 ease-out
                                        ${isAttackPressed
                                ...
                                >
                                    ...
                                    <img
                                        ...
                                        alt="Attack"
                                        className={`w-11 h-11 object-contain transition-all duration-100 pointer-events-none ${isAttackPressed ? 'opacity-100 scale-110 brightness-150' : 'opacity-50'}`}
                                    />
                                </button>
                            </div>
                        )}
