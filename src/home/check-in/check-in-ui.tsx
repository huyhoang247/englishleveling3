...
const DailyCheckInView = () => {
  ...
  return (
    ...
    // Thay đổi 1: Thay đổi class của div gốc.
    // - Bỏ `px-4 pt-4` ở các div con và đưa padding ra ngoài div gốc này.
    // - `pt-8` hoặc `pt-10` để tạo khoảng trống cho thanh trạng thái của điện thoại.
    // - `px-4` để tạo khoảng đệm hai bên.
    // - Bỏ `h-screen` ở div bên trong để nó co giãn theo nội dung.
    <div className="bg-black/90 w-full h-screen overflow-hidden pt-8 px-4">
      <div className="relative flex flex-col h-full">
        {/* --- BƯỚC 1: TẠO HEADER MỚI GIỐNG VỚI VOCA-CHEST --- */}
        <header className="flex-shrink-0 w-full h-[53px] box-border flex items-center justify-between bg-slate-900/70 backdrop-blur-sm border-b border-white/10 z-20 rounded-t-xl">
          {/* Thêm padding ngang cho nội dung header */}
        ...
        {/* --- BƯỚC 2: ĐƯA TOÀN BỘ NỘI DUNG CŨ VÀO MỘT CONTAINER CÓ THỂ CUỘN --- */}
        {/* Bỏ padding ở đây vì đã đưa ra ngoài div gốc */}
        <div className="flex-1 overflow-y-auto hide-scrollbar bg-slate-900/20 rounded-b-xl">
          <div className="p-4">
            <div className="flex justify-center mt-2 mb-6">
              ...
              <div className="bg-slate-900/80 rounded-xl px-4 py-4 w-full max-w-sm flex items-center gap-4 border border-slate-700 shadow-lg relative">
                ...
                {/* HomeButton đã được chuyển lên header mới */}
              </div>
            </div>
            <div className="mb-6">
            ...
            {/* --- PHẦN DANH SÁCH PHẦN THƯỞNG (ĐÃ NẰM TRONG CONTAINER CUỘN) --- */}
            <div className="pb-6">
