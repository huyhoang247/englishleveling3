        ...
        <div className="fixed inset-0 flex items-center justify-center z-[60] p-4">
            ...
            <div className="relative bg-gradient-to-br from-[#1a1c2e] to-[#0f111a] p-0 rounded-2xl border border-slate-600 shadow-2xl w-full max-w-4xl flex flex-col md:flex-row overflow-hidden max-h-[90vh]">
                ...
                <div className="flex-1 p-6 flex flex-col relative">
                    <button onClick={onClose} className="absolute top-4 right-4 text-slate-500 hover:text-white"><CloseIcon className="w-6 h-6" /></button>
                    
                    <div className="flex-1 flex flex-col justify-center">
                        <h4 className="text-slate-300 font-semibold mb-4 text-center">Chọn Đá Cường Hoá</h4>
                            ...
                            {stones.map((tier) => {
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`cursor-pointer relative p-4 rounded-xl border-2 transition-all duration-300 flex flex-col items-center gap-2 group
                                            ${isSelected 
                                                ? `bg-slate-800/80 ${stone.color.replace('text-', 'border-')} shadow-lg shadow-${stone.color.split('-')[1]}-500/20` 
                                                : 'bg-slate-900/40 border-slate-700 hover:border-slate-500'
                                            }
                                        `}
                                    >
                                        {/* Stone Icon Placeholder */}
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center bg-black/30 ${stone.color} font-bold text-2xl mb-1`}>
                                            {tier === 'low' ? 'I' : tier === 'medium' ? 'II' : 'III'}
                                        </div>
                                        <span className={`font-bold text-sm ${isSelected ? 'text-white' : 'text-slate-400'}`}>{stone.name}</span>
                                        <span className={`text-xs font-mono ${stone.color}`}>{stone.successRate * 100}%</span>
                                    </div>
                                );
                            })}
                        </div>
                        ...
                        {/* Thông tin xác suất */}
                        <div className="bg-black/30 rounded-lg p-4 mb-6 border border-slate-700/50 text-center">
                            <p className="text-slate-400 text-sm mb-1">{ENHANCEMENT_STONES[selectedStone].description}</p>
                            <p className="text-xs text-slate-500 italic">Thất bại sẽ giữ nguyên cấp độ nhưng mất đá.</p>
                        </div>
                        ...
                        <div className="flex justify-center">
                            <button 
                                ...
                                className={`
                                    relative overflow-hidden group w-full max-w-xs py-4 rounded-xl font-black text-lg uppercase tracking-widest transition-all duration-200
                                    ${isProcessing ? 'bg-slate-700 cursor-wait' : 'bg-gradient-to-r from-purple-600 to-indigo-600 hover:scale-105 hover:shadow-xl hover:shadow-purple-500/30 text-white'}
                                `}
                            >
                                <span className="relative z-10">{isProcessing ? 'Đang Xử Lý...' : 'Cường Hoá'}</span>
                    ...
                    {/* Status Overlay (Success/Fail Animation) */}
                    {upgradeStatus !== 'idle' && (
                        <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/60 backdrop-blur-[2px] rounded-r-2xl">
                            <div className={`transform transition-all duration-500 scale-100 ${upgradeStatus === 'success' ? 'text-green-400' : 'text-red-500'}`}>
                                <h2 className="text-5xl font-black uppercase drop-shadow-2xl animate-bounce">
                                    {upgradeStatus === 'success' ? 'Thành Công!' : 'Thất Bại!'}
                                </h2>
                            </div>
                        </div>
                    )}
            ...
            <div className={`relative bg-gradient-to-br ${getRarityGradient(itemDef.rarity)} p-5 rounded-xl border-2 ${getRarityColor(itemDef.rarity)} shadow-2xl w-full max-w-md max-h-[95vh] z-50 flex flex-col`}>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto hide-scrollbar pr-2 pb-2">
                    <div className="flex flex-col items-center text-center gap-4">
                        ...
                        {/* Chỉ hiển thị Stats, không còn Tab */}
                        {hasStats && (
                            <div className="w-full bg-black/20 rounded-lg overflow-hidden p-3 space-y-2">
                                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 border-b border-slate-700 pb-1 text-left">Thuộc tính</h4>
