...
const VocabularyChestLoadingSkeleton: React.FC = () => {
  return (
    ...
    <div className="w-full h-full absolute top-0 left-0 bg-gradient-to-br from-[#16213e] to-[#0a0a14] flex flex-col overflow-hidden">
      ...
      {/* THAY ĐỔI: Đồng bộ padding và gap chính xác với component thật */}
      <main className="flex-grow overflow-y-auto px-5 pt-5 pb-[100px]">
        <div className="w-full max-w-7xl mx-auto flex flex-wrap justify-center gap-[30px]">
          
          {[...Array(4)].map((_, i) => (
            // === THAY ĐỔI QUAN TRỌNG NHẤT: Thêm chiều cao CỐ ĐỊNH h-[365px] để khớp với card thật ===
            <div 
              key={i} 
              className="bg-slate-800/50 rounded-2xl overflow-hidden border border-white/10 flex flex-col w-full max-w-[380px] min-w-[300px] h-[365px] flex-shrink-0"
            >
              {/* Header của card */}
              <div className="h-[42px] py-3 px-5 bg-black/20 flex justify-center items-center flex-shrink-0">
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <div className="flex items-center gap-4 mb-5">
                  <div className="w-[90px] h-[90px] bg-white/10 animate-pulse rounded-lg flex-shrink-0"></div>
                  <div className="flex-1 h-[90px] bg-white/10 animate-pulse rounded-lg"></div>
                </div>

                {/* Phần các nút bấm (dùng mt-auto để đẩy xuống dưới) */}
                <div className="flex items-center gap-2.5 mt-auto pt-4">
                  <div className="flex-1 h-11 bg-white/10 animate-pulse rounded-lg"></div>
                  <div className="flex-1 h-11 bg-white/10 animate-pulse rounded-lg"></div>
                </div>
              </div>
            </div>
