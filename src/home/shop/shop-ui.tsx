...
const Tag = (props: any) => ( <img src="https://raw.githubusercontent.com/huyhoang247/englishleveling3/refs/heads/main/src/assets/images/item-icon.webp" alt="Items" {...props} /> );
...
const ShopHeader = ({ onClose, userGold, userGems, isLoading }: { onClose: () => void; userGold: number; userGems: number; isLoading: boolean; }) => { const navItems = ['Cửa Hàng', 'Nhiệm Vụ', 'Bang Hội', 'Sự Kiện']; const activeNav = 'Cửa Hàng'; const animatedGold = useAnimateValue(userGold, 500); const animatedGems = useAnimateValue(userGems, 500); return ( <header className="sticky top-0 left-0 right-0 z-40 bg-slate-900 border-b border-white/10"> <div className="max-w-[1600px] mx-auto flex items-center justify-between h-[53px] px-4 sm:px-6 lg:px-8"> <div className="flex items-center gap-4"> <HomeButton onClick={onClose} label="" title="Về trang chính" /> <nav className="hidden md:flex items-center gap-4"> {navItems.map(item => ( <a key={item} href="#" className={`text-sm font-medium px-3 py-2 rounded-md transition-colors ${ activeNav === item ? 'text-white bg-white/10' : 'text-slate-400 hover:text-white' }`}> {item} </a> ))} </nav> </div> <div className="flex items-center gap-3"> <GemDisplay displayedGems={isLoading ? 0 : animatedGems} /> <CoinDisplay displayedCoins={isLoading ? 0 : animatedGold} isStatsFullscreen={false} /> </div> </div> </header> ); };
...
const PaymentQRModal = ({ pkg, transaction, onClose, onConfirmPayment }: { pkg: any | null; transaction: any | null; onClose: () => void; onConfirmPayment: () => void; }) => {
    ...
    if (!pkg || !transaction) return null;

    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-3">
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm" onClick={onClose}></div>
            <div className="relative bg-gradient-to-br from-slate-900 to-purple-900/50 p-5 rounded-xl border-2 border-purple-500 shadow-2xl w-full max-w-md max-h-[95vh] z-50 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                    <h3 className="text-2xl font-bold text-purple-300">Thanh toán Gói Gems</h3>
                ...
                <div className="text-center w-full mx-auto flex-1 flex flex-col justify-between overflow-y-auto scrollbar-hide">
                    <div>
                        <h4 className="text-xl font-bold text-white mb-1">Thanh toán cho gói: {pkg.gems.toLocaleString()} Gems</h4>
                        <p className="text-2xl font-bold text-purple-400 mb-4">{pkg.price.toLocaleString('vi-VN')} VNĐ</p>
                        ...
                        <div className="mt-4 text-sm text-slate-300 space-y-3">
                            ...
                            <div className="bg-slate-800/60 p-3 rounded-lg border border-slate-700">
                                ...
                                <div className="flex items-center justify-between gap-2 mt-1">
                                    <p className="text-base font-mono font-bold text-yellow-300 tracking-widest">{transaction.transactionId}</p>
                                    <button onClick={handleCopyMemo} className="flex items-center gap-1.5 text-xs bg-slate-700 hover:bg-slate-600 text-slate-200 font-semibold px-2 py-1 rounded-md transition-colors"> <ClipboardCopy className="w-3 h-3" /> {copyText} </button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="mt-6">
                        <button
                            ...
                            className="w-full bg-gradient-to-r from-green-500 to-teal-600 text-white font-bold text-base uppercase px-5 py-3 rounded-lg transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-green-500/25 active:scale-100 disabled:from-gray-500 disabled:to-gray-600 disabled:cursor-not-allowed disabled:shadow-none disabled:scale-100"
                        >
                            {isConfirming ? 'ĐANG GỬI...' : (canConfirm ? 'XÁC NHẬN ĐÃ CHUYỂN TIỀN' : `VUI LÒNG CHỜ (${countdown}s)`)}
                        </button>
                        <p className="text-xs text-slate-400 mt-2">Nhấn nút này SAU KHI bạn đã hoàn tất chuyển khoản.</p>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <section>
                        {activeCategory === 'Nạp Gems' ? (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 gap-4 md:gap-6">
                                {allItems
                                    .filter(item => {
                                        if (['Item', 'Gói Đặc Biệt', 'Sự Kiện'].includes(activeCategory)) {
                                            return ['Item', 'Rương', 'Trang phục'].includes(item.type);
                                        }
                                        if (activeCategory === 'Vũ Khí') return item.type === 'Vũ khí';
                                        if (activeCategory === 'Trang Bị') return item.type === 'Trang bị';
                                        return false;
                                    })
                                    .map(item => (
                                        <ShopItemCard
