...
import { useVirtualizer } from '@tanstack/react-virtual'; // THÊM IMPORT
  ...
  return (
    <div id={`flashcard-${card.id}`} className="flex flex-col items-center bg-white dark:bg-gray-800 shadow-xl overflow-hidden relative group">
      ...
      <div className="absolute top-3 right-3 z-10 flex items-center space-x-2">
        <button className={`transition-[transform,background-color] duration-300 flex items-center justify-center p-1.5 bg-white/80 dark:bg-gray-900/80 rounded-full shadow-md hover:bg-white dark:hover:bg-gray-900 ${isFavorite ? 'scale-110' : 'scale-100'}`}
...
export default function VerticalFlashcardGallery({ hideNavBar, showNavBar, currentUser }: VerticalFlashcardGalleryProps) {
  ...
  // VIRTUALIZER: Khởi tạo hook virtualizer
  const rowVirtualizer = useVirtualizer({
    count: layoutMode === 'single' 
        ? filteredFlashcardsByTab.length 
        : Math.ceil(filteredFlashcardsByTab.length / 2),
    getScrollElement: () => mainContainerRef.current,
    estimateSize: () => layoutMode === 'single' ? 750 : 400, // Chiều cao ước tính của một hàng
    overscan: 5, // Render thêm 5 item ngoài viewport để cuộn mượt hơn
  });
                ...
                {filteredFlashcardsByTab.length > 0 ? (
                  ...
                  >
                    {rowVirtualizer.getVirtualItems().map((virtualItem) => {
                      ...
                      const itemsInRow = isSingleColumn ? [filteredFlashcardsByTab[virtualItem.index]] : [
                          filteredFlashcardsByTab[virtualItem.index * 2],
                          filteredFlashcardsByTab[virtualItem.index * 2 + 1]
                      ].filter(Boolean);
                      ...
                      return (
                        <div
                          ...
                          className={`flex ${isSingleColumn ? '' : 'gap-4'}`}
                        >
                          {itemsInRow.map((displayCard) => (
                             <div key={displayCard.card.id} className={`py-2 ${isSingleColumn ? 'w-full' : 'w-1/2'}`}>
                                <FlashcardItem
                                  card={displayCard.card}
                                  isFavorite={displayCard.isFavorite}
