...
const BookIcon = ({ className = '' }: { className?: string }) => (
    <img src="https://raw.githubusercontent.com/huyhoang247/englishleveling3/refs/heads/main/src/icon/20250720_1859_Icon%20S%C3%A1ch%20C%E1%BB%95%20Anime_simple_compose_01k0kv0rg5fhzrx8frbtsgqk33.png" alt="Sách Cổ" className={className} />
);
...
const GoldIcon = ({ className = '' }: { className?: string }) => (
    <img src="https://raw.githubusercontent.com/huyhoang247/englishleveling3/refs/heads/main/src/icon/dollar.png" alt="Vàng" className={className} />
);
...
const SkillCard = ({ ownedSkill, onClick, isEquipped, isProcessing }: { ownedSkill: OwnedSkill, onClick: () => void, isEquipped: boolean, isProcessing: boolean }) => {
    ...
    const interactivity = isProcessing ? 'cursor-wait' : `cursor-pointer hover:border-slate-600 hover:bg-slate-800/50 hover:shadow-lg hover:shadow-cyan-500/10`;
    const IconComponent = skillBlueprint.icon;

    return (
        <div className={`${baseClasses} border-slate-700 bg-slate-900/70 ${interactivity}`} onClick={!isProcessing ? onClick : undefined}>
            {isEquipped && (
                <div className="absolute inset-0 bg-black/40 rounded-lg z-10 flex items-center justify-center text-xs font-bold uppercase tracking-widest text-cyan-400">
                    Equipped
                </div>
            )}
            ...
            <div className="flex-grow flex flex-col justify-center">
                <div className="flex justify-between items-center">
                    ...
                    <span className={`px-2 py-0.5 text-xs font-bold rounded-full bg-slate-800 border ${getRarityColor(ownedSkill.rarity)} ${getRarityTextColor(ownedSkill.rarity)}`}>
                        {ownedSkill.rarity}
                    </span>
                </div>
                <div className="mt-1">
...
const SkillDetailModal = ({ ownedSkill, onClose, onEquip, onUnequip, onDisenchant, onUpgrade, isEquipped, gold, isProcessing }: { ownedSkill: OwnedSkill, onClose: () => void, onEquip: (skill: OwnedSkill) => void, onUnequip: (skill: OwnedSkill) => void, onDisenchant: (skill: OwnedSkill) => void, onUpgrade: (skill: OwnedSkill) => void, isEquipped: boolean, gold: number, isProcessing: boolean }) => {
    ...
    const upgradeCost = isUpgradable ? getUpgradeCost(skill.upgradeCost!, ownedSkill.level) : 0;
    const canAffordUpgrade = isUpgradable && gold >= upgradeCost;
                ...
                <div className="flex-1 min-h-0 overflow-y-auto hide-scrollbar pr-2">
                    <div className="flex flex-col items-center text-center gap-4">
                        ...
                        <p className="text-slate-300 text-base leading-relaxed">
                            {skill.description(ownedSkill.level, ownedSkill.rarity)}
                        </p>
                                ...
                                >
                                    <div className="flex flex-col">
                                        ...
                                        <div className="flex items-center gap-2 font-bold text-lg mt-1">
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-purple-400 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                                    ...
                                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-md transition-colors ${!canAffordUpgrade ? 'bg-slate-700 border border-slate-600' : 'bg-slate-800 border border-slate-600 group-hover:bg-purple-600/50 group-hover:border-purple-500'}`}>
                                        ...
                                        <span className={`font-bold text-sm transition-colors ${!canAffordUpgrade ? 'text-slate-500' : 'text-yellow-300'}`}>{upgradeCost.toLocaleString()}</span>
                                    </div>
                                </button>
                ...
                <div className="flex-shrink-0 mt-auto border-t border-gray-700/50 pt-4">
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className={`flex-1 font-bold text-sm uppercase py-3 rounded-lg transition-all duration-300 transform ${actionDisabled ? mainActionDisabledStyle : mainActionStyle}`}
                        >
                            {mainActionText}
                        </button>
                        <button
                            onClick={() => onDisenchant(ownedSkill)}
                            disabled={isEquipped || actionDisabled}
                            className={`flex-1 font-bold text-sm uppercase py-3 rounded-lg transition-all duration-300 transform ${isEquipped || actionDisabled ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : 'bg-gradient-to-r from-red-500 to-orange-500 text-white hover:scale-105 hover:shadow-lg hover:shadow-red-500/25 active:scale-100'}`}
                        >
                            Phân Rã
                        </button>
                    </div>
...
export default function SkillScreen({ onClose, gold, ancientBooks, ownedSkills, equippedSkillIds, onSkillsUpdate }: SkillScreenProps) {
    ...
    const handleUnequipSkill = async (skillToUnequip: OwnedSkill) => {
        ...
        if (slotIndex === -1) {
            showMessage("Lỗi: Không tìm thấy kỹ năng đã trang bị.");
            return;
        }
        setIsProcessing(true);
        const newEquippedIds = [...equippedSkillIds];
    ...
    const handleUpgradeSkill = async (skillToUpgrade: OwnedSkill) => {
        ...
        const upgradeCost = getUpgradeCost(skillBlueprint.upgradeCost, skillToUpgrade.level);
        if (gold < upgradeCost) {
            showMessage(`Không đủ vàng. Cần ${upgradeCost.toLocaleString()}.`);
            return;
        }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 overflow-y-auto hide-scrollbar">
                            {ownedSkills.length > 0 ? (
                                ownedSkills
                                    ...
                                    .map(ownedSkill => (
                                        <SkillCard
                                            ...
                                            onClick={() => setSelectedSkill(ownedSkill)}
                                            isEquipped={false}
                                            isProcessing={isProcessing}
                                        />
                                    ))
                                ...
                                <div className="col-span-full flex items-center justify-center h-full text-slate-500">
                                    <p>Chưa có kỹ năng. Hãy dùng Sách Cổ để Craft!</p>
                                </div>
                            )}
                        </div>
                    </section>
