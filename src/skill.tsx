...
// Tối ưu hóa việc sử dụng class màu động với Tailwind
const colorMap = {
...
export default function SkillEquipScreen() {
  ...
  const showMessage = (text: string) => {
    setMessage(text);
    setMessageKey(prev => prev + 1); // Reset animation
  ...
  return (
    <div className="main-bg relative w-full min-h-screen bg-gradient-to-br from-[#110f21] to-[#2c0f52] font-sans text-white overflow-hidden">
      ...
      {/* Container chính cho toàn bộ nội dung, đảm bảo layout co giãn tốt */}
      <div className="relative z-10 flex flex-col w-full h-screen p-4 sm:p-6 md:p-8">
        ...
        <header className="w-full max-w-5xl mx-auto text-center flex-shrink-0 pb-4">
          <h1 className="text-3xl md:text-4xl font-black tracking-wider uppercase title-glow">Trang Bị Kỹ Năng</h1>
          <p className="text-slate-400 mt-1 text-sm md:text-base">Chọn kỹ năng để trang bị, hoặc nhấp vào ô đã chọn để gỡ bỏ.</p>
        </header>

        {/* --- Vùng Nội dung chính (Slot và Kho) --- */}
        <main className="w-full max-w-5xl mx-auto flex flex-col flex-grow min-h-0 gap-6">
            ...
            {/* --- KHO KỸ NĂNG (Scrollable) --- */}
            {/* Sử dụng flex-grow và min-h-0 để vùng này chiếm không gian còn lại và cho phép scroll */}
