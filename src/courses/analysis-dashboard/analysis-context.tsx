...
// ... (Các component Icon và component con không thay đổi)
...
const ChartCard: FC<{ title: string; children: ReactNode; extra?: ReactNode }> = ({ title, children, extra }) => ( <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-lg border border-gray-100 transition-all duration-300 hover:shadow-xl hover:-translate-y-1"> <div className="flex items-center justify-between mb-4"> <h3 className="text-lg font-bold text-gray-800">{title}</h3> {extra && <div>{extra}</div>} </div> <div className="h-64 sm:h-72 w-full">{children}</div> </div>);
...
interface MilestoneProgressProps { title: string; iconSrc: string; milestones: number[]; currentProgress: number; masteryCount: number; claimedMilestones: number[]; onClaim: (milestone: number, rewardAmount: number) => Promise<void>; user: User | null; progressColorClass?: string; completedText?: string; }
const MilestoneProgress: FC<MilestoneProgressProps> = memo(({ title, iconSrc, milestones, currentProgress, masteryCount, claimedMilestones, onClaim, user, progressColorClass = "from-green-400 to-blue-500", completedText = "All missions completed!" }) => { const [isClaiming, setIsClaiming] = useState(false); const { currentGoal, progressPercentage, isGoalMet, areAllGoalsMet } = useMemo(() => { const nextGoalIndex = milestones.findIndex(g => !claimedMilestones.includes(g)); if (nextGoalIndex === -1) return { areAllGoalsMet: true, progressPercentage: 100, isGoalMet: true, currentGoal: milestones[milestones.length - 1] }; const currentGoal = milestones[nextGoalIndex]; const previousGoal = nextGoalIndex > 0 ? milestones[nextGoalIndex - 1] : 0; const progressInMilestone = Math.max(0, currentProgress - previousGoal); const milestoneSize = currentGoal - previousGoal; const progressPercentage = milestoneSize > 0 ? Math.min((progressInMilestone / milestoneSize) * 100, 100) : 100; const isGoalMet = currentProgress >= currentGoal; return { currentGoal, progressPercentage, isGoalMet, areAllGoalsMet: false }; }, [currentProgress, claimedMilestones, milestones]); const handleClaim = useCallback(async () => { if (!isGoalMet || areAllGoalsMet || !user || isClaiming || !currentGoal) return; setIsClaiming(true); try { const rewardAmount = currentGoal * Math.max(1, masteryCount); await onClaim(currentGoal, rewardAmount); } catch (error) { console.error(`Lỗi khi nhận thưởng cho "${title}":`, error); alert("Đã có lỗi xảy ra. Vui lòng thử lại."); } finally { setIsClaiming(false); } }, [isGoalMet, areAllGoalsMet, user, isClaiming, currentGoal, masteryCount, onClaim, title]); return ( <div className="bg-white p-4 sm:p-5 rounded-2xl shadow-lg border border-gray-100 transition-all duration-300 hover:shadow-xl hover:-translate-y-1"> <div className="flex items-center justify-between gap-4 flex-wrap"> <div className="flex items-center gap-3 flex-shrink-0"> <img src={iconSrc} alt={title} className="w-11 h-11" /> <div> <h3 className="text-base sm:text-lg font-bold text-gray-800">{title}</h3> {areAllGoalsMet ? ( <p className="text-xs sm:text-sm text-green-600 font-semibold">{completedText}</p> ) : ( <div className="flex items-center text-xs sm:text-sm text-gray-500 mt-1" title={`Reward = Milestone (${currentGoal}) × Max(1, Mastery Cards: ${masteryCount})`}> <span className="flex items-center font-bold text-amber-600"> <img src={uiAssets.goldIcon} alt="Reward Coin" className="h-5 w-5 mr-1.5"/> <span className="text-sm">{currentGoal * Math.max(1, masteryCount)}</span> </span> </div> )} </div> </div> <div className="flex-shrink-0"> {areAllGoalsMet ? ( <div className="text-center p-2 bg-green-100 text-green-700 rounded-lg flex items-center gap-2"><CheckCircleIconSmall /> <span className="font-bold text-sm">Awesome!</span></div> ) : isGoalMet ? ( <button onClick={handleClaim} disabled={isClaiming} className="flex items-center justify-center px-4 py-2 font-bold text-white bg-gradient-to-r from-yellow-400 via-orange-500 to-red-500 rounded-lg shadow-md hover:scale-105 transform transition-transform duration-200 disabled:opacity-70 disabled:cursor-wait"> {isClaiming ? <SpinnerIcon /> : <GiftIcon />} <span className="ml-1.5 text-sm">{isClaiming ? 'Wait' : 'Claim'}</span> </button> ) : ( <div className="flex items-center justify-center gap-2 px-4 py-2 font-bold bg-gray-200 text-gray-500 rounded-lg cursor-not-allowed"> <GiftIcon /> <span className="flex items-baseline"><span className="text-base font-extrabold text-gray-700">{currentProgress}</span><span className="text-sm font-medium text-gray-500">/{currentGoal}</span></span> </div> )} </div> <div className="w-full mt-3"> {areAllGoalsMet ? ( <div className="h-2.5 w-full bg-gradient-to-r from-green-400 to-teal-500 rounded-full" title="All milestones completed!"></div> ) : ( <div className={`w-full bg-gray-200 rounded-full h-2.5`}><div className={`bg-gradient-to-r ${progressColorClass} h-2.5 rounded-full transition-all duration-500 ease-out`} style={{ width: `${progressPercentage}%` }}></div></div> )} </div> </div> </div> ); });
const ActivityCalendar: FC<{ activityData: any }> = memo(({ activityData }) => { const formatDateForCalendar = (date: Date): string => { const year = date.getFullYear(); const month = (date.getMonth() + 1).toString().padStart(2, '0'); const day = date.getDate().toString().padStart(2, '0'); return `${year}-${month}-${day}`; }; const calendarData = useMemo(() => { const today = new Date(); today.setHours(0, 0, 0, 0); const dayOfWeek = today.getDay(); const startOfWeek = new Date(today); startOfWeek.setDate(today.getDate() - (dayOfWeek === 0 ? 6 : dayOfWeek - 1)); const startDate = new Date(startOfWeek); startDate.setDate(startOfWeek.getDate() - (4 * 7)); const days = []; const activityDates = new Set(Object.keys(activityData)); for (let i = 0; i < 35; i++) { const date = new Date(startDate); date.setDate(startDate.getDate() + i); const dateString = formatDateForCalendar(date); const hasActivity = activityDates.has(dateString); const activityDetail = activityData[dateString] || { new: 0, review: 0 }; days.push({ date, dateString, dayOfMonth: date.getDate(), hasActivity, isToday: date.getTime() === today.getTime(), isFuture: date.getTime() > today.getTime(), tooltip: hasActivity ? `${date.toLocaleDateString('vi-VN')}: Học ${activityDetail.new} từ mới, ôn ${activityDetail.review} từ.` : date.toLocaleDateString('vi-VN'), }); } return days; }, [activityData]); const weekDayHeaders = ['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN']; return ( <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-lg border border-gray-100 transition-all duration-300 hover:shadow-xl hover:-translate-y-1"> <div className="flex items-center gap-3 mb-4"> <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-2 rounded-lg shadow-md"> <CalendarIcon /> </div> <h3 className="text-lg font-bold text-gray-800">Activity</h3> </div> <div className="grid grid-cols-7 gap-1.5 sm:gap-2 text-center"> {weekDayHeaders.map(day => <div key={day} className="text-xs font-semibold text-gray-500 mb-2">{day}</div>)} {calendarData.map((day, index) => { const baseClass = "w-full aspect-square rounded-lg flex items-center justify-center text-sm font-semibold transition-all duration-200 ease-in-out"; let dayClass = ""; if (day.isFuture) { dayClass = "bg-gray-100 text-gray-400 cursor-not-allowed"; } else { dayClass = "bg-gray-200 text-gray-600 hover:bg-gray-300"; } if (day.isToday) { dayClass += " ring-2 ring-offset-2 ring-indigo-500"; } return ( <div key={index} title={day.tooltip} className={`${baseClass} ${dayClass}`}> {(day.hasActivity && !day.isFuture) ? <ActivityCompletedIcon /> : <span>{day.dayOfMonth}</span> } </div> ); })} </div> <div className="flex items-center justify-center sm:justify-end gap-4 mt-4 text-xs text-gray-600"> <div className="flex items-center gap-2"><div className="w-3.5 h-3.5 rounded-md bg-gray-200"></div><span>Chưa học</span></div> <div className="flex items-center gap-2"> <ActivityCompletedIcon className="h-4 w-4" /> <span>Đã học</span> </div> <div className="flex items-center gap-2"><div className="w-3.5 h-3.5 rounded-md ring-2 ring-offset-1 ring-indigo-500"></div><span>Hôm nay</span></div> </div> </div> ); });
const CustomTooltip = ({ active, payload, label }: any) => { if (active && payload && payload.length) { const finalLabel = payload[0].payload.game ? payload[0].payload.game : `Ngày: ${label}`; const total = payload.reduce((sum, entry) => sum + entry.value, 0); return ( <div className="p-2 bg-gray-800 text-white rounded-md shadow-lg text-sm border border-gray-700"> <p className="font-bold">{finalLabel}</p> {payload.map((pld) => <p key={pld.dataKey} style={{ color: pld.fill }}>{`${pld.name}: ${pld.value}`}</p>)} {payload.length > 1 && total > 0 && <><hr className="my-1 border-gray-600" /><p className="font-semibold">{`Tổng: ${total}`}</p></>} </div> ); } return null; };
                        ...
                        <div className="max-w-7xl mx-auto">
                            <div className="space-y-6 my-6">
                                <MilestoneProgress 
                                    ...
                                    // [SỬA] Sử dụng prop mới từ context
                                    claimedMilestones={userProgress.localClaimedVocabMilestones} 
                                ...
                                <MilestoneProgress 
                                    ...
                                    masteryCount={userProgress.masteryCount} 
                                    // [SỬA] Sử dụng prop mới từ context
                                    claimedMilestones={userProgress.localClaimedDailyGoals} 
