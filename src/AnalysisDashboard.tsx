...
interface VocabularyMilestonesProps { totalWordsLearned: number; masteryCount: number; user: User | null; claimedVocabMilestones: number[]; onClaimSuccess: (milestone: number, rewardAmount: number) => void; }
interface DailyGoalMilestonesProps { wordsLearnedToday: number; masteryCount: number; user: User | null; claimedDailyGoals: number[]; onClaimSuccess: (milestone: number, rewardAmount: number) => void; }
...
// --- VocabularyMilestones Component ---
...
const VocabularyMilestones: FC<VocabularyMilestonesProps> = memo(({ totalWordsLearned, masteryCount, user, claimedVocabMilestones, onClaimSuccess }) => {
    ...
    const { currentGoal, progressPercentage, isGoalMet, areAllGoalsMet } = useMemo(() => {
        ...
        if (nextGoalIndex === -1) { return { areAllGoalsMet: true, progressPercentage: 100, isGoalMet: true, currentGoal: VOCAB_MILESTONES[VOCAB_MILESTONES.length - 1], previousGoal: 0 }; }
...
const DailyGoalMilestones: FC<DailyGoalMilestonesProps> = memo(({ wordsLearnedToday, masteryCount, user, claimedDailyGoals, onClaimSuccess }) => {
    ...
    const { currentGoal, progressPercentage, isGoalMet, areAllGoalsMet } = useMemo(() => {
        ...
        if (nextGoalIndex === -1) { return { areAllGoalsMet: true, progressPercentage: 100, isGoalMet: true, currentGoal: GOAL_MILESTONES[GOAL_MILESTONES.length - 1], previousGoal: 0 }; }
    ...
    const handleClaim = useCallback(async () => {
        ...
        try {
            const rewardAmount = currentGoal * Math.max(1, masteryCount);
            await claimDailyGoalReward(user.uid, currentGoal, rewardAmount);
            onClaimSuccess(currentGoal, rewardAmount);
        } catch (error) {
...
// --- Component chính AnalysisDashboard ---
...
export default function AnalysisDashboard({ onGoBack }: AnalysisDashboardProps) {
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      ...
      setError(null);
      try {
        const { analysisData, dailyActivityMap, userStats: fetchedUserStats } = await fetchDashboardData(
            user.uid,
            defaultVocabulary.length
        );

        setAnalysisData(analysisData);
        setDailyActivityData(dailyActivityMap);
        setUserStats({
          coins: fetchedUserStats.coins,
          masteryCount: fetchedUserStats.masteryCount,
        });
        setClaimedDailyGoals(fetchedUserStats.claimedDailyGoals);
        setClaimedVocabMilestones(fetchedUserStats.claimedVocabMilestones);
