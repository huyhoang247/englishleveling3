...
// --- >>> START: COMPONENT CATEGORY TABS ĐÃ ĐƯỢC CẬP NHẬT <<< ---
const CategoryTabs = ({ activeCategory, setActiveCategory }: { activeCategory: string; setActiveCategory: (category: string) => void }) => {
    const categories = [ { name: 'Nạp Gems', icon: Gem }, { name: 'Đổi Vàng', icon: Coins }, { name: 'Item', icon: Tag }, { name: 'Vũ Khí', icon: Swords }, { name: 'Trang Bị', icon: Shield }, { name: 'Gói Đặc Biệt', icon: Sparkles }, { name: 'Sự Kiện', icon: ArrowRightCircle }, ];
        ...
        <>
            ...
            <style jsx>{`
                ...
                nav::after {
                    ...
                    width: 50px; background: linear-gradient(to left, #0a0a14, transparent);
                    pointer-events: none;
                }
            `}</style>
        </>
...
const GameShopUI = ({ onClose, onPurchaseComplete }: { onClose: () => void; onPurchaseComplete: () => void; }) => {
    ...
    return (
        <div className="w-full h-screen bg-[#0a0a14] font-sans text-white flex flex-col">
                    ...
                    <section>
                        {activeCategory === 'Nạp Gems' ? (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 gap-4 md:gap-6">
                                {allItems
                                    .filter(item => {
                                        if (activeCategory === 'Item' || activeCategory === 'Gói Đặc Biệt' || activeCategory === 'Sự Kiện') {
                                        ...
                                        return false; // Mặc định không hiển thị gì nếu category không khớp
                                    })
                                    .map(item => (
                                        <ShopItemCard
            ...
            {/* Các Modal được giữ nguyên ở cuối để không bị ảnh hưởng bởi layout */}
