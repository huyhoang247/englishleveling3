...
const Gem = (props) => ( <Icon {...props}><path d="M6 3h12l4 6-10 13L2 9z"/><path d="M12 22 6 9l-4-6"/><path d="M12 22 18 9l4-6"/></Icon> );
const Swords = (props) => ( <Icon {...props}><path d="M14.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4-4L14.5 3.5z"/><path d="M19.5 8.5a2.12 2.12 0 0 1-3-3L10 12l4 4L19.5 8.5z"/></Icon> );
...
const Sparkles = (props) => ( <Icon {...props}><path d="m12 3-1.9 4.2-4.3.4 3.3 2.9-1 4.2 3.6-2.3 3.6 2.3-1-4.2 3.3-2.9-4.3-.4L12 3z"/><path d="M5 12.5 3.1 14 5 15.5"/><path d="M19 12.5 20.9 14 19 15.5"/></Icon> );
...
const Tag = (props) => ( <Icon {...props}><path d="M12.586 2.586A2 2 0 0 0 11.172 2H4a2 2 0 0 0-2 2v7.172a2 2 0 0 0 .586 1.414l8.704 8.704a2.426 2.426 0 0 0 3.432 0l6.568-6.568a2.426 2.426 0 0 0 0-3.432L12.586 2.586z"/><circle cx="7.5" cy="7.5" r=".5" fill="currentColor"/></Icon> );
// --- LOGIC MỚI: Thêm icon cho countdown timer ---
...
// --- THAY ĐỔI: Dữ liệu mẫu cho cửa hàng (chỉ giữ lại các vật phẩm không phải vũ khí) ---
const sampleItemsNonWeapons = [
    { id: 2, name: 'Giáp Thiên Thần', type: 'Trang bị', rarity: 'Sử thi', price: 1820, image: 'https://placehold.co/600x600/1a1a2e/87ceeb?text=🛡️', description: 'Bộ giáp mang lại sự bảo vệ tối thượng và khả năng hồi phục máu theo thời gian.' },
    { id: 6, name: 'Khiên Bất Diệt', type: 'Trang bị', rarity: 'Sử thi', price: 2000, image: 'https://placehold.co/600x600/1a1a2e/c0c0c0?text=🛡️', description: 'Một chiếc khiên không thể bị phá hủy, chặn mọi đòn tấn công từ phía trước.' },
    { id: 4, name: 'Gói Trang Phục Hắc Tinh', type: 'Trang phục', rarity: 'Sử thi', price: 2200, image: 'https://placehold.co/600x600/1a1a2e/9370db?text=✨', description: 'Thay đổi ngoại hình của bạn thành một thực thể vũ trụ bí ẩn và quyền năng.' },
    { id: 3, name: 'Ngọc Tái Sinh', type: 'Vật phẩm', rarity: 'Hiếm', price: 975, image: 'https://placehold.co/600x600/1a1a2e/32cd32?text=💎', description: 'Hồi sinh ngay lập tức tại chỗ khi bị hạ gục. Chỉ có thể sử dụng một lần mỗi trận.' },
    { id: 7, name: 'Vé Nâng Cấp VIP', type: 'Vật phẩm', rarity: 'Phổ thông', price: 500, image: 'https://placehold.co/600x600/1a1a2e/f0e68c?text=🎟️', description: 'Nhận đặc quyền VIP trong 30 ngày, bao gồm tăng kinh nghiệm và vật phẩm nhận được.' },
    { id: 8, name: 'Rương Kho Báu Bí Ẩn', type: 'Rương', rarity: 'Hiếm', price: 750, image: 'https://placehold.co/600x600/1a1a2e/d2b48c?text=📦', description: 'Mở để có cơ hội nhận được một vật phẩm quý hiếm ngẫu nhiên từ danh sách phần thưởng.' },
];

// --- THAY ĐỔI: Cấu hình rarity mới theo hệ thống Rank E -> SR ---
const rarityConfig = {
    ...
    // Giữ lại các rank cũ để các vật phẩm khác không bị lỗi
...
// --- LOGIC MỚI: Logic tạo vật phẩm ngẫu nhiên cho cửa hàng vũ khí ---
...
const generateDailyShopWeapons = () => {
    // 1. Lấy tất cả vũ khí từ database
    const allWeapons = Array.from(itemDatabase.values()).filter(item => item.type === 'weapon');
    
    // 2. Xáo trộn và lấy 10 vũ khí ngẫu nhiên
    const selectedWeapons = shuffleArray(allWeapons).slice(0, 10);

    // 3. Gán rank và giá ngẫu nhiên cho mỗi vũ khí
    const shopItems = selectedWeapons.map(weapon => {
        ...
        return {
            id: weapon.id,
            name: weapon.name,
            type: 'Vũ khí', // Dùng cho category filter
            rarity: randomRank,
            price: price,
            image: `https://placehold.co/600x600/1a1a2e/ffffff?text=${encodeURIComponent(weapon.icon)}`,
...
const getShopItems = () => {
    try {
        ...
        if (storedData && storedTimestamp && parseInt(storedTimestamp, 10) === today) {
            console.log("Loading shop data from localStorage.");
            return JSON.parse(storedData);
        } else {
            console.log("Generating new shop data for today.");
...
const ShopItemCard = ({ item, onSelect }) => {
    const config = rarityConfig[item.rarity] || rarityConfig['E'];
    return (
        <div 
        ...
        <nav className="flex flex-wrap gap-2 mb-8">
            {categories.map(({ name, icon: Icon }) => (
                <button key={name} onClick={() => setActiveCategory(name)}
                    className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 transform hover:-translate-y-0.5 ${ activeCategory === name ? 'bg-cyan-500 text-slate-900 shadow-lg shadow-cyan-500/20' : 'bg-slate-800 text-slate-300 hover:bg-slate-700 hover:text-white' }`}>
...
// --- LOGIC MỚI: Component Countdown Timer ---
const ShopCountdown = () => {
    ...
    useEffect(() => {
        const calculateTimeLeft = () => {
            const now = new Date();
            // Lấy thời gian UTC của nửa đêm ngày mai
        ...
        calculateTimeLeft(); // Chạy lần đầu ngay lập tức
...
const GameShopUI = ({ onClose }) => {
    ...
    // --- THAY ĐỔI: Sử dụng state để chứa tất cả vật phẩm ---
                ...
                <main>
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
                            {gridItems.map(item => (
                                <ShopItemCard key={`${item.id}-${item.name}`} item={item} onSelect={handleSelectItem} />
                            ))}
                        </div>
                    </section>
                </main>
