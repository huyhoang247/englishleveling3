        ...
        <header className="sticky top-0 left-0 right-0 z-40 bg-slate-900/80 backdrop-blur-md border-b border-slate-700/60 transition-all duration-300">
            <div className="max-w-[1600px] mx-auto flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
                <div className="flex items-center gap-4">
                    <button onClick={onClose} className="w-10 h-10 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-700/60 hover:text-white transition-colors" aria-label="Đóng cửa hàng"> <img src={uiAssets.closeIcon} alt="Close" className="w-5 h-5" /> </button>
                    <nav className="hidden md:flex items-center gap-4"> {navItems.map(item => ( <a key={item} href="#" className={`text-sm font-medium px-3 py-2 rounded-md transition-colors ${ activeNav === item ? 'text-white bg-white/10' : 'text-slate-400 hover:text-white' }`}> {item} </a> ))} </nav>
                </div>
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 bg-slate-800/60 backdrop-blur-sm pl-2 pr-3 py-1.5 rounded-full border border-slate-700/50">
                        <Coins className="w-6 h-6" />
                        {isLoading ? (
                            <div className="w-16 h-4 bg-slate-700 rounded-md animate-pulse"></div>
                        ) : (
                            <span className="font-bold text-sm text-yellow-300 tracking-wider">{userGold.toLocaleString()}</span>
                        )}
                    </div>
                    <div className="flex items-center gap-2 bg-slate-800/60 backdrop-blur-sm pl-2 pr-3 py-1.5 rounded-full border border-slate-700/50">
                        ...
                        {isLoading ? (
                            ...
                            <span className="font-bold text-sm text-cyan-300 tracking-wider">{userGems.toLocaleString()}</span>
                        )}
                    </div>
                </div>
            </div>
