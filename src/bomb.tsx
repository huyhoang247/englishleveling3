...
// --- Các component Icon SVG (giữ nguyên các icon cũ) ---
...
// --- THAY ĐỔI: Thêm icon mũi tên để phân biệt bom ---
const ArrowHorizontalIcon = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={className}>
        <path d="M21 12H3"/><path d="m12 5-7 7 7 7"/>
    </svg>
);
const ArrowVerticalIcon = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={className}>
        <path d="M12 21V3"/><path d="m5 12 7-7 7 7"/>
    </svg>
);


// --- THAY ĐỔI: Cấu hình game với loại bom mới ---
const BOARD_SIZE = 6;
...
export default function App() {
  ...
  // --- THAY ĐỔI: Cập nhật hàm khởi tạo bàn chơi với loại bom mới ---
  function createBoard() {
    const newBoard = Array(BOARD_SIZE).fill(null).map((_, rowIndex) =>
      Array(BOARD_SIZE).fill(null).map((_, colIndex) => ({
        x: colIndex, y: rowIndex,
        isMineHorizontal: false, // Thay thế isMine
        isMineVertical: false,   // Thay thế isMine
        isCoin: false, isExit: false,
        isRevealed: false, isFlagged: false,
      }))
    );

    // Hàm phụ trợ để tìm một ô trống ngẫu nhiên
    const placeItem = (itemType) => {
        ...
        while(!placed) {
            ...
            // Ô được coi là trống nếu không có bất cứ thứ gì khác
  ...
  // --- THAY ĐỔI: Hàm xử lý vụ nổ hàng ngang ---
  function explodeRow(board, y, initialClick = false) {
    let chainReaction = false;
    let newCoins = 0;
    for (let i = 0; i < BOARD_SIZE; i++) {
        const cell = board[y][i];
        if (cell.isRevealed) continue; // Bỏ qua ô đã mở

        // Game over nếu vụ nổ kích hoạt một quả bom khác
        if (!initialClick || i !== cell.x) { // Không tính quả bom được click ban đầu
    ...
    if(chainReaction) setGameOver(true);
  }

  // --- THAY ĐỔI: Hàm xử lý vụ nổ hàng dọc ---
  function explodeColumn(board, x, initialClick = false) {
    let chainReaction = false;
    let newCoins = 0;
    for (let i = 0; i < BOARD_SIZE; i++) {
  ...
  // --- THAY ĐỔI: Hàm mở ô trở thành một "dispatcher" ---
  function revealCell(currentBoard, x, y) {
    const cell = currentBoard[y][x];
    if (cell.isRevealed || cell.isFlagged) return;

    // Phân loại và xử lý
    if (cell.isMineHorizontal) {
        explodeRow(currentBoard, y, true);
    } else if (cell.isMineVertical) {
        explodeColumn(currentBoard, x, true);
    } else {
        // Xử lý ô thường (vàng, lối thoát, ô trống)
        cell.isRevealed = true;
        if (cell.isCoin) {
  ...
  // Component Cell
  const Cell = ({ cellData }) => {
    const { x, y, isRevealed, isMineHorizontal, isMineVertical, isCoin, isFlagged, isExit } = cellData;

    const cellStyle = {
      base: 'w-full h-full rounded-lg transition-all duration-200 relative', // Thêm relative
    ...
    if (isFlagged) {
      ...
      if (isMineHorizontal || isMineVertical) {
        // --- THAY ĐỔI: Hiển thị bom với icon chỉ hướng ---
        content = (
            <div className="relative w-full h-full flex items-center justify-center">
                 ...
                 {isMineVertical && <ArrowVerticalIcon className="absolute w-1/2 h-1/2 text-red-500 opacity-80" />}
            </div>
        );
      } else if (isExit) {
    ...
    <main className="bg-slate-900 text-white min-h-screen flex flex-col items-center justify-center p-4 font-poppins">
      <div className="w-full max-w-xs sm:max-w-sm mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-slate-400 mt-2">Kích hoạt bom và tránh nổ dây chuyền!</p>
        </div>
        ...
        <div className="relative">
          <div className="w-full aspect-square">
            <div 
              ...
              style={{ gridTemplateColumns: `repeat(${BOARD_SIZE}, 1fr)`, gap: '6px' }}
            >
              {board.map((row) => row.map((cell) => <Cell key={`${cell.y}-${cell.x}`} cellData={cell} />))}
            </div>
          </div>
          ...
          {gameOver && (
            <div className="absolute inset-0 bg-black/70 flex items-center justify-center rounded-xl backdrop-blur-sm">
              <div className="text-center bg-slate-800 p-8 rounded-xl shadow-2xl border border-slate-700 animate-fade-in">
                    ...
                    <h2 className="text-3xl font-bold text-red-500 mb-2">NỔ DÂY CHUYỀN!</h2>
                    {/* --- THAY ĐỔI: Thông báo game over mới --- */}
                    <p className="text-slate-300">Vụ nổ đã kích hoạt một quả bom khác.</p>
                ...
                <button onClick={resetGame} className="mt-6 bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-8 rounded-lg transition-all">Chơi lại</button>
              </div>
            </div>
          )}

          {/* Lớp phủ xác nhận lên tầng */}
          {showExitConfirmation && (
             ...
             <div className="text-center bg-slate-800 p-8 rounded-xl shadow-2xl border border-slate-700 animate-fade-in">
                   ...
                   <p className="text-slate-400 text-sm mt-1">Bạn vẫn có thể ở lại để dọn dẹp bàn chơi.</p>
               <div className="mt-6 flex justify-center gap-4">
                <button onClick={() => setShowExitConfirmation(false)} className="bg-slate-600 hover:bg-slate-500 text-white font-bold py-3 px-8 rounded-lg">Ở lại</button>
                <button onClick={goToNextFloor} className="bg-green-600 hover:bg-green-500 text-white font-bold py-3 px-8 rounded-lg">Lên tầng</button>
               </div>
             </div>
           </div>
          )}
