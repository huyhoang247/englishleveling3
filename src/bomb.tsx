...
// --- Các component Icon SVG (Thêm icon cho bom ngang/dọc) ---
...
export default function App() {
  ...
  function createBoard(currentLevel) {
    ...
    for (let i = 0; i < NUM_HORIZONTAL_BOMBS; i++) {
        placeItem((cell) => isCellEmpty(cell), (cell) => cell.bombType = 'horizontal');
    }
    for (let i = 0; i < NUM_VERTICAL_BOMBS; i++) {
        placeItem((cell) => isCellEmpty(cell), (cell) => cell.bombType = 'vertical');
    }
    for (let i = 0; i < totalCoinsOnBoard; i++) {
        placeItem((cell) => isCellEmpty(cell), (cell) => cell.isCoin = true);
    }
    placeItem((cell) => isCellEmpty(cell), (cell) => cell.isStairs = true);

    return newBoard;
  }

  function handleCellClick(x, y) {
    if (gameOver || levelCleared || board[y][x].isFlagged) return;

    const cell = board[y][x];

    // LOGIC MỚI: Click vào Lối Thoát đã mở để qua tầng
    if (cell.isRevealed && cell.isStairs) {
        if (coinsOnThisLevel >= config.coinsRequired) {
            setLevelCleared(true);
        }
        return; // Dừng lại, không xử lý gì thêm
    }
    
    if(cell.isRevealed) return; // Không cho click vào ô đã mở khác
  ...
  function triggerLinedExplosion(currentBoard, x, y, bombType) {
    if (bombType === 'horizontal') {
        for (let i = 0; i < BOARD_SIZE; i++) {
            revealCell(currentBoard, i, y, true); // `fromExplosion = true` để tránh đệ quy vô hạn
        }
    } else if (bombType === 'vertical') {
        for (let i = 0; i < BOARD_SIZE; i++) {
            revealCell(currentBoard, x, i, true);
        }
    }
  }
  ...
  function revealCell(currentBoard, x, y, fromExplosion = false) {
    ...
    if (cell.isCoin) {
      ...
      // Dừng, không mở rộng khi gặp vàng
      return;
    }
    
    if (cell.isStairs) {
      // Dừng, chỉ mở ô cầu thang, không mở rộng
      return;
    }
    ...
    // Mở rộng các ô trống xung quanh (chỉ khi không phải nổ bom)
    if (cell.bombType === 'none' && !cell.isCoin && !cell.isStairs) {
  ...
  const Cell = ({ cellData }) => {
    ...
    if (isFlagged) {
      ...
      if (bombType === 'horizontal') {
        content = <HorizontalBombIcon className="w-3/4 h-3/4 text-white animate-pulse" />;
      } else if (bombType === 'vertical') {
        content = <VerticalBombIcon className="w-3/4 h-3/4 text-white animate-pulse" />;
      } else if (isStairs) {
        content = <StairsIcon className={`w-3/4 h-3/4 transition-colors duration-500 ${canPass ? 'text-green-400 cursor-pointer hover:scale-110' : 'text-slate-500'}`} />
      } else if (isCoin) {
    ...
    <main className="bg-slate-900 text-white min-h-screen flex flex-col items-center justify-center p-4 font-poppins">
      <div className="w-full max-w-md sm:max-w-lg mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-slate-400 mt-2">Đạt <span className="text-yellow-400 font-bold">{config.coinsRequired} vàng</span> để mở khóa Lối Thoát!</p>
        </div>
...
style.innerHTML = `body { font-family: 'Poppins', sans-serif; } @keyframes fade-in { from { opacity: 0; transform: scale(0.9); } to { opacity: 1; transform: scale(1); } } .animate-fade-in { animation: fade-in 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards; }`;
