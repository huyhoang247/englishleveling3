    ...
    try {
        ...
        if (selectedType === "tracNghiem") {
            ...
            allFillModes.forEach((mode) => {
                ...
                const practiceMode = practiceNum % 100;
                ...
                } else if ([3, 4, 5, 6, 7].includes(practiceMode)) {
                    // Xác định số từ yêu cầu trong câu
                    let requiredWords = 0;
                    if (practiceMode === 7) {
                        requiredWords = 1; // P7 cần ít nhất 1 từ
                    } else { // P3, P4, P5, P6
                        requiredWords = practiceMode - 1; // P3 cần 2, P4 cần 3...
                    }
        ...
        functions.logger.error("Error calculating progress:", error);
        throw new functions.https.HttpsError(
            "internal",
